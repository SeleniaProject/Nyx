...
async fn retry_succeeds_before_max_attempts() {
    ...
    let res: Result<u32, &'static str> = retry(
        {
            let n = n.clone();
            move || {
                let value = n.clone();
...
#[tokio::test]
async fn retry_propagates_error_after_max_attempts() {
    let n = Arc::new(AtomicU32::new(0));
    let res: Result<u32, &'static str> = retry(
        {
            let n = n.clone();
            move || {
                let value = n.clone();
                async move { let _ = value.fetch_add(1, Ordering::SeqCst); Err("nope") }
            }
        },
        |_| 1,
        3,
...
#[test]
fn backoff_policy_exponential_with_jitter_bounds() {
