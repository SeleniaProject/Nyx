...
fn protocol_code_passthrough() {
    let e = NyxError::Protocol {
        message: "Server said nope".into(),
        code: Some(0x07),
    }; // pretend UNSUPPORTED_CAP
...
fn unsupported_cap_in_message_detected() {
    let e = NyxError::Protocol {
        message: "UNSUPPORTED_CAP capability id=42".into(),
        code: None,
    };
    assert_eq!(e.close_code(), Some(0x07));
}
