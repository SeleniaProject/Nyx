...
impl NyxConfig {
    ...
    pub async fn load<P: AsRef<Path>>(path: P) -> NyxResult<Self> {
        ...
        let content = fs::read_to_string(path).await.map_err(|e| {
            NyxError::config_error(
                format!("Failed to read config file: {}", e),
                Some(path.to_string_lossy().to_string()),
            )
        })?;

        let config: NyxConfig = toml::from_str(&content).map_err(|e| {
            NyxError::config_error(
    ...
    pub fn validate(&self) -> NyxResult<()> {
        ...
        if self.daemon.connect_timeout.is_zero() {
            return Err(NyxError::invalid_input(
                "connect_timeout must be > 0",
                Some("daemon.connect_timeout".to_string()),
            ));
        }
        ...
        if self.daemon.request_timeout.is_zero() {
            return Err(NyxError::invalid_input(
                ...
                Some("daemon.request_timeout".to_string()),
            ));
        }
        ...
        if self.network.max_paths == 0 || self.network.max_paths > 8 {
            return Err(NyxError::invalid_input(
                ...
                Some("network.max_paths".to_string()),
            ));
        }
        ...
        if self.network.retry.max_attempts == 0 {
            return Err(NyxError::invalid_input(
                ...
                Some("network.retry.max_attempts".to_string()),
            ));
        }
        ...
        if self.network.retry.backoff_multiplier <= 1.0 {
            return Err(NyxError::invalid_input(
                ...
                Some("network.retry.backoff_multiplier".to_string()),
            ));
        }
        ...
        if self.streams.buffer_size == 0 {
            return Err(NyxError::invalid_input(
                ...
                Some("streams.buffer_size".to_string()),
            ));
        }
        ...
        if self.streams.max_streams == 0 {
            return Err(NyxError::invalid_input(
                ...
                Some("streams.max_streams".to_string()),
            ));
        }
    ...
    pub async fn save<P: AsRef<Path>>(&self, path: P) -> NyxResult<()> {
        let content = toml::to_string_pretty(self).map_err(|e| {
            NyxError::config_error(format!("Failed to serialize config: {}", e), None)
        })?;
