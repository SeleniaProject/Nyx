...
pub mod backoff_policy {
	...
	pub fn exponential_with_jitter(__attempt: u32, __base_m_s: u64, max_m_s: u64) -> Duration {
		let __pow = if __attempt >= 64 { 0 } else { 1u64.checked_shl(__attempt.min(16)).unwrap_or(0) };
...
pub mod backoff_policy {
	use std::time::Duration;
	pub fn exponential_with_jitter(___attempt: u32, ___base_m_s: u64, _max_m_s: u64) -> Duration { Duration::from_millis(0) }
}
