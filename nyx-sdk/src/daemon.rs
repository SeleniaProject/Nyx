...
impl DaemonClient {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if let Err(e) = read_one_line(&mut stream_inner, &mut tmp).await {
                    let _ = tx.send(Event { event_type: "system".into(), detail: format!("events_stream_closed:{e}") });
                    break;
                }
                ...
                match serde_json::from_slice::<Event>(&tmp) {
                    ...
                    Err(e) => {
                        if tx.send(Event { event_type: "system".into(), detail: format!("events_decode_error:{e}") }).is_err() { break; }
                    }
                }
            }
        });
    ...
    async fn rpc_json<T: for<'de> Deserialize<'de>>(&self, req: &RpcRequest<'_>) -> Result<T> {
        ...
        let mut buf = Vec::with_capacity(1024);
        read_one_line_with_timeout(&mut stream, &mut buf, self.cfg.request_timeout_ms).await?;
        let resp: RpcResponseValue = serde_json::from_slice(&buf).map_err(|e| Error::Protocol(e.to_string()))?;
        if resp.ok {
...
async fn read_one_line<R: AsyncRead + Unpin>(reader: &mut R, out: &mut Vec<u8>) -> Result<()> {
    ...
    out.clear();
    loop {
        let n = reader.read(&mut tmp).await.map_err(|e| Error::Stream(e.to_string()))?;
        if n == 0 { break; }
...
async fn read_one_line_with_timeout<R: AsyncRead + Unpin>(reader: &mut R, out: &mut Vec<u8>, timeout_ms: u64) -> Result<()> {
    ...
    loop {
        ...
        let n = timeout(remain, reader.read(&mut buf)).await.map_err(|_| Error::Timeout)?.map_err(|e| Error::Stream(e.to_string()))?;
        if n == 0 { break; }
