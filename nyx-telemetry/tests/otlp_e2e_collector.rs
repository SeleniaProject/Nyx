...
impl TraceService for MockTraceService {
    ...
    ) -> Result<Response<ExportTraceServiceResponse>, Statu_s> {
        let req = request.into_inner();
        let mut count = 0usize;
        for r_s in req.resource_span_s {
            for s_s in r_s.scope_span_s {
                count = count.saturating_add(s_s.span_s.len());
            }
        }
        let _ = self.tx.send(count).await;
        Ok(Response::new(ExportTraceServiceResponse {
            partial_succes_s: None,
        }))
    }
}
...
async fn start_mock_collector(
    bind: SocketAddr,
) -> (SocketAddr, oneshot::Sender<()>, mpsc::Receiver<usize>) {
    ...
    let incoming = tokio_stream::wrapper_s::TcpListenerStream::new(tokio_listener);
...
async fn otlp_exporter_flushes_to_mock_collector() {
    ...
    let deadline = tokio::time::Instant::now() + Duration::from_sec_s(3);
