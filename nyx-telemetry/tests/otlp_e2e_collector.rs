...
use nyx_telemetry::opentelemetry_integration::{NyxTelemetry, TelemetryConfig as OCfg};
...
struct MockTrace {
    store: Arc<Mutex<Vec<ExportTraceServiceRequest>>>,
}
#[tonic::async_trait]
...
async fn otlp_exporter_sends_span_and_attributes_preserved() {
    ...
    // Emit a handshake span to validate exporter preserves pq_mode attribute
    {
        let hs = tracing::span!(tracing::Level::INFO, "nyx.handshake", pq_mode = "classic");
        let _g = hs.enter();
    }
                ...
                for sp in &ss.spans {
                    if sp.name == "nyx.stream.send" {
                        ...
                        for kv in &sp.attributes {
                            if kv.key == "path_id" {
                                if let Some(anyv) = &kv.value {
                                    if let Some(inner) = &anyv.value {
                                        match inner {
                                            Value::IntValue(i) => pid = Some(*i),
                                            Value::StringValue(s) => {
                            ...
                            if kv.key == "cid" {
                                if let Some(anyv) = &kv.value {
                                    if let Some(inner) = &anyv.value {
                                        if let Value::StringValue(s) = inner {
                                            cid = Some(s.clone());
                                        }
                                    }
                                }
                    ...
                    if sp.name == "nyx.handshake" {
                        // pq_mode should be present and equal to "classic"
                        let mut pq_mode: Option<String> = None;
                        for kv in &sp.attributes {
                            if kv.key == "pq_mode" {
                                if let Some(anyv) = &kv.value {
                                    if let Some(Value::StringValue(s)) = &anyv.value {
                                        pq_mode = Some(s.clone());
                                    }
                                }
                            }
                        }
                        assert_eq!(pq_mode.as_deref(), Some("classic"));
                        found_hs = true;
                    }
                }
            }
    ...
    assert!(
        found_hs,
        "expected handshake span not found in exported data"
    );
}
