...
pub fn set_attribute_filter(f: Option<Arc<AttrFilterFn>>) {
    TL_ATTR_FILTER.with(|cell| *cell.borrow_mut() = f);
}
...
impl<'a> MapVisitor<'a> {
    fn store(&mut self, k: &str, v: &str) {
        TL_ATTR_FILTER.with(|cell| {
            if let Some(f) = &*cell.borrow() {
                if let Some(nv) = (f)(k, v) {
                    self.map.insert(k.to_string(), nv);
                    return;
                } else {
                    return;
                }
...
impl InMemorySpanLayer {
    fn new(store: SharedCapturedSpans) -> Self {
        Self {
            store,
...
use tokio::sync::mpsc;
#[cfg(feature = "otlp_exporter")]
static EXPORT_SENDER: OnceLazy<std::sync::RwLock<Option<mpsc::Sender<CapturedSpan>>>> =
    OnceLazy::new(|| std::sync::RwLock::new(None));
#[cfg(feature = "otlp_exporter")]
pub fn register_export_sender(tx: mpsc::Sender<CapturedSpan>) {
    *EXPORT_SENDER.write().unwrap() = Some(tx);
}
            ...
            if let Some(span_ref) = ctx.span(&id) {
                ...
                if name == "nyx.stream.send" {
                    if let Some(pid) = map.get("path_id").and_then(|v| v.parse::<u8>().ok()) {
                        if let Some(h) = SPAN_PATH_HOOK.read().unwrap().as_ref() {
                            (h)(pid);
                        }
                    }
                }
...
static SPAN_PATH_HOOK: Lazy<StdRwLock<Option<StdArc<SpanPathHook>>>> =
    Lazy::new(|| StdRwLock::new(None));
pub fn register_span_path_hook<F: Fn(u8) + Send + Sync + 'static>(f: F) {
    *SPAN_PATH_HOOK.write().unwrap() = Some(StdArc::new(f));
}

pub fn init_in_memory_tracer(
    _service_name: &str,
    sampling_ratio: f64,
) -> (tracing::Dispatch, SharedCapturedSpans) {
    let store: SharedCapturedSpans = Arc::new(Mutex::new(Vec::new()));
    struct SamplingLayer {
        inner: InMemorySpanLayer,
        ratio: f64,
    }
        ...
        S: Subscriber + for<'a> tracing_subscriber::registry::LookupSpan<'a>,
    {
        fn on_new_span(&self, attrs: &tracing::span::Attributes<'_>, id: &Id, ctx: Context<'_, S>) {
            ...
            if !deterministic_accept(&SPAN_COUNTER, self.ratio) {
                return;
            }
    ...
    let subscriber = Registry::default().with(SamplingLayer {
        inner: InMemorySpanLayer::new(store.clone()),
...
static EXPORT_SUCCESS: Lazy<std::sync::RwLock<Option<IntCounter>>> =
    Lazy::new(|| std::sync::RwLock::new(None));
static EXPORT_FAILURE: Lazy<std::sync::RwLock<Option<IntCounter>>> =
    Lazy::new(|| std::sync::RwLock::new(None));
static EXPORT_CIRCUIT: Lazy<std::sync::RwLock<Option<IntGauge>>> =
    Lazy::new(|| std::sync::RwLock::new(None));
...
impl CircuitBreaker {
    fn on_success(&mut self) {
        ...
        if self.open {
            self.open = false;
            self.opened_at = None;
            if let Some(g) = &*EXPORT_CIRCUIT.read().unwrap() {
                g.set(0);
            }
        }
    }
    ...
    fn allow(&mut self) -> bool {
        ...
        if let Some(t) = self.opened_at {
            if t.elapsed() >= Duration::from_secs(self.open_secs) {
                self.open = false;
                self.fail_count = 0;
                self.opened_at = None;
                if let Some(g) = &*EXPORT_CIRCUIT.read().unwrap() {
    ...
    Fut: std::future::Future<Output = anyhow::Result<T>>,
{
    ...
    for _ in 0..20 {
        ...
        match attempt().await {
            Ok(v) => {
                breaker.on_success();
                if let Some(c) = &*EXPORT_SUCCESS.read().unwrap() {
                    c.inc();
                }
                return Some(v);
            }
            Err(_) => {
                breaker.on_failure();
                if let Some(c) = &*EXPORT_FAILURE.read().unwrap() {
                    c.inc();
                }
                tokio::time::sleep(delay).await;
                delay = Duration::min(
                    backoff.max,
                    Duration::from_secs_f64(delay.as_secs_f64() * backoff.factor),
                );
            }
        }
    }
...
mod tests {
    ...
    async fn sampling_ratio_zero_drops_span() {
        ...
        tracing::dispatcher::with_default(&dispatch, || {
            ...
            let _g = span.enter();
            tracing::info!("should not record");
        });
        let stored = spans.lock().unwrap();
    ...
    async fn init_only_no_crash() {
        ...
        assert!(
            store.is_empty(),
            "span store should be empty right after initialization"
        );
    }
    ...
    async fn ratio_sampling_drops_some() {
        ...
        assert!(
            stored > 0 && stored < 200,
            "deterministic sampling should keep subset: {}",
            stored
        );
    }
