...
#[cfg(feature = "otlp")]
use tracing_subscriber::{layer::SubscriberExt, util::SubscriberInitExt}; // for .with_timeout()
...
pub fn init_tracing(servicename: Option<String>) -> anyhow::Result<()> {
    ...
    let svcname = servicename.unwrap_or_else(|| "nyx".to_string());
    ...
    let use_batch =
        tokio::runtime::Handle::try_current().is_ok() && schedule_delay > Duration::from_millis(0);
    eprintln!(
        "nyx-telemetry:init schedule_delay_ms={} use_batch={}",
        schedule_delay.as_millis(),
        use_batch
    );
    // SimpleSpanProcessor if used, switch sampler to AlwaysOff to avoid hang during export
    // to handle test environment hangs.
    if !use_batch {
...
pub fn shutdown() {
    if let Some(p) = TRACER_PROVIDER.get() {
        // SimpleSpanProcessor does not need shutdown wait for test environments.
...
pub fn init_tracing(_servicename: Option<String>) -> anyhow::Result<()> {
    if tracing::dispatcher::has_been_set() {
        return Ok(());
    }
