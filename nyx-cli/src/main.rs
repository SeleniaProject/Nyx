...
async fn auto_discover() -> (SdkConfig, Option<String>) {
	...
	// 1) Env vars
	if let Ok(ep) = std::env::var("NYX_DAEMON_ENDPOINT") { let e = ep.trim(); if !e.is_empty() { cfg.daemon_endpoint = e.to_string(); } }
...
fn parse_cli_toml(s: &str) -> Option<CliFileConfig> {
	...
	if let Some(cli) = v.get("cli") {
		if let Some(ep) = cli.get("daemon_endpoint").and_then(|x| x.as_str()) { let ep = ep.trim(); if !ep.is_empty() { out.endpoint = Some(ep.to_string()); } }
		if let Some(tok) = cli.get("token").and_then(|x| x.as_str()) { let tok = tok.trim(); if !tok.is_empty() { out.token = Some(tok.to_string()); } }
		if let Some(ms) = cli.get("request_timeout_ms").and_then(|x| x.as_integer()) {
			if ms >= 0 { out.timeout_ms = Some(ms as u64); }
		}
	}
	Some(out)
}
