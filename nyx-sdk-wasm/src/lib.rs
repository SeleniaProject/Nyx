...
use web_sys::{
    window, PushManager, PushSubscription, PushSubscriptionOptionsInit, ServiceWorkerRegistration,
};
mod errors;
#[cfg(feature = "hpke")]
mod hpke;
...
pub use errors::{nyx_check_required_plugins, nyx_map_close_code};
#[cfg(feature = "hpke")]
pub use hpke::{
    hpke_generate_and_seal_session, hpke_generate_keypair, hpke_open, hpke_open_session, hpke_seal,
};
pub use management::{nyx_build_close_unsupported_cap, nyx_build_plugin_settings};
pub use multipath::{MultipathConfigWasm, MultipathController, PathSelectionResult, PathStatsWasm};
...
#[wasm_bindgen]
pub async fn nyx_register_push(
    sw_path: String,
    ...
    let key_buf = general_purpose::URL_SAFE_NO_PAD
        .decode(vapid_public_key.as_bytes())
...
pub fn nyx_multipath_controller_new(config_json: Option<String>) -> MultipathController {
    let cfg = config_json.map(|s| {
        MultipathConfigWasm::new(Some(s))
            .unwrap_or_else(|_| MultipathConfigWasm::new(None).unwrap())
    });
