...
use serde::Serialize;
use web_sys::{Request, Headers, Response, window};
...
#[wasm_bindgen]
pub fn nyx_build_plugin_settings(required_cbor_b64: String, support_flags: u32, security_policy: u32) -> Result<js_sys::Uint8Array, JsValue> {
    ...
    settings.push(Setting { id: setting_ids::PLUGIN_SUPPORT, value: support_flags });
    settings.push(Setting { id: setting_ids::PLUGIN_SECURITY_POLICY, value: security_policy });
    settings.push(Setting { id: setting_ids::PLUGIN_REQUIRED, value: count });
...
#[wasm_bindgen]
pub fn nyx_compute_plugin_support_flags(dynamic_loading: bool, sandboxed_execution: bool, inter_plugin_ipc: bool, persistence: bool) -> u32 {
    let mut flags = plugin_support_flags::BASIC_FRAMES;
    if dynamic_loading { flags |= plugin_support_flags::DYNAMIC_LOADING; }
    if sandboxed_execution { flags |= plugin_support_flags::SANDBOXED_EXECUTION; }
    if inter_plugin_ipc { flags |= plugin_support_flags::INTER_PLUGIN_IPC; }
    if persistence { flags |= plugin_support_flags::PLUGIN_PERSISTENCE; }
    flags
}

/// Compute plugin security policy flags.
#[wasm_bindgen]
pub fn nyx_compute_plugin_security_policy(require_signatures: bool, allow_network: bool, allow_filesystem: bool, allow_inter_plugin_ipc: bool, allow_process_spawn: bool) -> u32 {
    let mut policy = 0u32;
    if require_signatures { policy |= plugin_security_flags::REQUIRE_SIGNATURES; }
    if allow_network { policy |= plugin_security_flags::ALLOW_NETWORK; }
    if allow_filesystem { policy |= plugin_security_flags::ALLOW_FILESYSTEM; }
    if allow_inter_plugin_ipc { policy |= plugin_security_flags::ALLOW_INTER_PLUGIN_IPC; }
    if allow_process_spawn { policy |= plugin_security_flags::ALLOW_PROCESS_SPAWN; }
    policy
}
...
#[wasm_bindgen]
pub fn nyx_prepare_plugin_handshake(required_cbor_b64: String, support_flags: u32, security_policy: u32) -> Result<JsValue, JsValue> {
    let settings = nyx_build_plugin_settings(required_cbor_b64.clone(), support_flags, security_policy)?;
...
#[wasm_bindgen]
pub async fn nyx_send_plugin_settings(url: String, required_cbor_b64: String, support_flags: u32, security_policy: u32) -> Result<JsValue, JsValue> {
    ...
    headers.append("Content-Type", "application/nyx-settings").map_err(|e| JsValue::from(e))?;
...
#[wasm_bindgen]
pub async fn nyx_send_close(url: String, close_payload: js_sys::Uint8Array) -> Result<JsValue, JsValue> {
    let headers = Headers::new().map_err(|e| JsValue::from(e))?;
    headers.append("Content-Type", "application/nyx-close").map_err(|e| JsValue::from(e))?;
    let win = window().ok_or(JsValue::from_str("no window"))?;
    let mut init = js_sys::Object::new();
    js_sys::Reflect::set(&init, &JsValue::from_str("method"), &JsValue::from_str("POST"))?;
    ...
    let request_js = request_ctor.call2(&JsValue::NULL, &JsValue::from_str(&url), &init)?;
    let request: Request = request_js.dyn_into()?;
    request.headers().set("Content-Type", "application/nyx-close")?;
    let resp_value = JsFuture::from(win.fetch_with_request(&request)).await?;
    let resp: Response = resp_value.dyn_into()?;
    let status = resp.status();
    let ok = resp.ok();
...
#[wasm_bindgen]
pub async fn nyx_set_required_plugins(base_url: String, required_cbor_b64: String) -> Result<JsValue, JsValue> {
    let win = window().ok_or(JsValue::from_str("no window"))?;
    // Build JSON body { required_cbor_b64: "..." }
    let obj = js_sys::Object::new();
    js_sys::Reflect::set(&obj, &JsValue::from_str("required_cbor_b64"), &JsValue::from_str(&required_cbor_b64))?;
    let json = js_sys::JSON::stringify(&obj).map_err(|e| JsValue::from(e))?;
    let mut init = js_sys::Object::new();
    js_sys::Reflect::set(&init, &JsValue::from_str("method"), &JsValue::from_str("POST"))?;
    js_sys::Reflect::set(&init, &JsValue::from_str("body"), &json)?;
    let request_ctor = js_sys::Function::new_with_args("u,i", "return new Request(u,i);");
    let url = format!("{}/api/v1/wasm/handshake/required", base_url.trim_end_matches('/'));
    let request_js = request_ctor.call2(&JsValue::NULL, &JsValue::from_str(&url), &init)?;
    let request: Request = request_js.dyn_into()?;
