...
pub fn hpke_generate_keypair() -> Result<JsValue, JsValue> {
    #[cfg(feature = "hpke")]
    {
        ...
        let kp = HpkeKeypair { sk: b64e(&<core_hpke::PrivateKey as Serializable>::to_bytes(&sk)), pk: b64e(&<core_hpke::PublicKey as Serializable>::to_bytes(&pk)) };
...
#[wasm_bindgen]
pub fn hpke_seal(pk_b64: String, info_b64: String, aad_b64: String, pt_b64: String) -> Result<JsValue, JsValue> {
    #[cfg(feature = "hpke")]
    {
        ...
        let (enc, ct) = core_hpke::seal(&pk, &info, &aad, &pt).map_err(|_| JsValue::from_str("hpke seal failed"))?;
        let out = HpkeSealResult { enc: b64e(&enc), ct: b64e(&ct) };
...
#[wasm_bindgen]
pub fn hpke_open(sk_b64: String, enc_b64: String, info_b64: String, aad_b64: String, ct_b64: String) -> Result<JsValue, JsValue> {
    #[cfg(feature = "hpke")]
    {
        ...
        let pt = core_hpke::open(&sk, &enc, &info, &aad, &ct).map_err(|_| JsValue::from_str("hpke open failed"))?;
...
#[wasm_bindgen]
pub fn hpke_generate_and_seal_session(pk_b64: String, info_b64: String, aad_b64: String) -> Result<JsValue, JsValue> {
    #[cfg(feature = "hpke")]
    {
        let pk_bytes = b64d(&pk_b64)?;
        let info = b64d(&info_b64)?;
...
#[wasm_bindgen]
pub fn hpke_open_session(sk_b64: String, enc_b64: String, info_b64: String, aad_b64: String, ct_b64: String) -> Result<String, JsValue> {
    #[cfg(feature = "hpke")]
    {
        let sk_bytes = b64d(&sk_b64)?;
        let enc = b64d(&enc_b64)?;
