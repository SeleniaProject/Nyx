...
impl PluginRegistryWasm {
    ...
    pub fn new() -> PluginRegistryWasm { PluginRegistryWasm { manifests: Vec::new() } }
    ...
    pub fn add_manifest(&mut self, manifest_json: String) -> Result<(), JsValue> {
        ...
        // Basic checks
        if manifest.name.trim().is_empty() { return Err(JsValue::from_str("Manifest name empty")); }
        if Version::parse(&manifest.version).is_err() { return Err(JsValue::from_str("Manifest version not semver")); }
    ...
    pub fn verify_all(&self) -> Result<JsValue, JsValue> {
        let results: Vec<ValidationResult> = self.manifests.iter().map(|m| verify_manifest(m)).collect();
    ...
    pub fn export_required_plugins_cbor_b64(&self) -> Result<String, JsValue> {
        let required_ids: Vec<u32> = self.manifests.iter()
            .filter(|m| m.required)
            .map(|m| m.id)
            .collect();
        let mut buf = Vec::new();
...
fn verify_manifest(m: &PluginManifest) -> ValidationResult {
    ...
    let Some(sig_b64) = &m.signature_b64 else {
        return ValidationResult { ok: true, message: "No signature attached".to_string() };
    };
    let Some(pk_b64) = &m.registry_pubkey_b64 else {
        return ValidationResult { ok: false, message: "Missing registry public key".to_string() };
    };
    let Ok(sig_bytes) = base64::engine::general_purpose::STANDARD.decode(sig_b64) else {
        return ValidationResult { ok: false, message: "Invalid base64 signature".to_string() };
    };
    let Ok(pk_bytes) = base64::engine::general_purpose::STANDARD.decode(pk_b64) else {
        return ValidationResult { ok: false, message: "Invalid base64 public key".to_string() };
    };
    let Ok(pk_arr) = <[u8; 32]>::try_from(pk_bytes.as_slice()) else {
        return ValidationResult { ok: false, message: "Invalid public key length".to_string() };
    };
    let Ok(vk) = VerifyingKey::from_bytes(&pk_arr) else {
        return ValidationResult { ok: false, message: "Invalid public key length".to_string() };
    };
    let Ok(sig_arr) = <[u8; 64]>::try_from(sig_bytes.as_slice()) else {
        return ValidationResult { ok: false, message: "Invalid signature length".to_string() };
    };
    let sig = Signature::from_bytes(&sig_arr);
    let msg = m.canonical_json();
    let digest = Sha256::digest(msg.as_bytes());
    match vk.verify(digest.as_slice(), &sig) {
        Ok(_) => ValidationResult { ok: true, message: "Signature valid".to_string() },
        Err(_) => ValidationResult { ok: false, message: "Signature verification failed".to_string() },
    }
}
