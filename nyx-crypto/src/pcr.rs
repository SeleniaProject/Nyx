...
pub fn derivenext_key(old: &[u8; 32], label: &[u8]) -> Result<[u8; 32]> {
    let hk = Hkdf::<Sha256>::from_prk(old).map_err(|_| Error::Crypto("HKDF from_prk failed".into()))?;
    let mut out = [0u8; 32];
    hk.expand(label, &mut out).map_err(|_| Error::Crypto("HKDF expand failed".into()))?;
    Ok(out)
}
