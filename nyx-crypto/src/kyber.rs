...
pub mod sizes {
	pub use pqc_kyber::{
		KYBER_CIPHERTEXTBYTES as CIPHERTEXT,
		KYBER_PUBLICKEYBYTES as PUBLIC_KEY,
		KYBER_SECRETKEYBYTES as SECRET_KEY,
		KYBER_SSBYTES as SHARED_SECRET,
	};
}

/// Public key bytes for Kyber.
pub type PublicKey = [u8; kyber_impl::KYBER_PUBLICKEYBYTES as usize];
/// Secret key bytes for Kyber.
pub type SecretKey = [u8; kyber_impl::KYBER_SECRETKEYBYTES as usize];
/// Ciphertext bytes for Kyber encapsulation.
pub type Ciphertext = [u8; kyber_impl::KYBER_CIPHERTEXTBYTES as usize];
/// Shared secret bytes.
pub type SharedSecret = [u8; kyber_impl::KYBER_SSBYTES as usize];

/// Deterministically derive a keypair from a 32-byte seed.
pub fn derive(seed: [u8; 32]) -> Result<(SecretKey, PublicKey)> {
	let kp = kyber_impl::derive(&seed)
		.map_err(|e| Error::Protocol(format!("kyber derive: {e}")))?;
...
pub fn keypair<R: CryptoRng + RngCore>(rng: &mut R) -> Result<(SecretKey, PublicKey)> {
	let kp = kyber_impl::keypair(rng)
		.map_err(|e| Error::Protocol(format!("kyber keypair: {e}")))?;
	Ok((kp.secret, kp.public))
}

/// Encapsulate to a public key, returning (ciphertext, shared_secret).
pub fn encapsulate<R: CryptoRng + RngCore>(pk: &PublicKey, rng: &mut R) -> Result<(Ciphertext, SharedSecret)> {
