...
pub fn validate_hybrid_message_len(msg: &[u8]) -> Result<()> {
	if msg.len() < 8 { return Err(Error::Protocol("hybrid message too short".into())); }
	if msg.len() > MAX_NOISE_MSG_LEN { return Err(Error::Protocol("hybrid message too long".into())); }
	Ok(())
}
...
pub mod ik_demo {
	...
	impl SymmetricState {
		fn h(data: &[u8]) -> [u8;32] { let mut d = sha2::Sha256::new(); d.update(data); d.finalize().into() }
		...
		fn mix_hash(&mut self, data: &[u8]) { let mut d = sha2::Sha256::new(); d.update(self.h); d.update(data); self.h = d.finalize().into(); }
		...
		fn aad_tag(&self, label: &[u8]) -> [u8;32] { let mut d = sha2::Sha256::new(); d.update(self.h); d.update(label); d.finalize().into() }
	}

	#[derive(Clone)]
	pub struct StaticKeypair { pub sk: [u8;32], pub pk: [u8;32] }
	impl StaticKeypair {
		pub fn generate() -> Self {
			let mut seed = [0u8;32]; rand::rngs::OsRng.fill_bytes(&mut seed);
			Self::from_seed(seed)
		}
	...
	#[derive(Debug)]
	pub struct ResponderResult { pub tx: AeadSession, pub rx: AeadSession, pub msg2: Vec<u8>, pub early_data: Option<Vec<u8>> }

	pub fn initiator_handshake(i_static: &StaticKeypair, r_static_pk: &[u8;32], prologue: &[u8]) -> Result<InitiatorResult> {
	...
	pub fn initiator_handshake_with_eph_seed(i_static: &StaticKeypair, r_static_pk: &[u8;32], prologue: &[u8], e_seed: [u8;32]) -> Result<InitiatorResult> {
		// 0-RTTなしをデフォルトで選択
	...
	pub fn initiator_handshake_with_eph_seed_0rtt(
		...
		if let Some(early_pt) = early {
			...
			let mut k_e = [0u8;32]; k_e.copy_from_slice(&out[..32]);
			let mut n_e = [0u8;12]; n_e.copy_from_slice(&out[32..]);
			out.zeroize();
			...
			// u16BE length + body
			let len_u16: u16 = ct_e.len().try_into().map_err(|_| Error::Protocol("early data too long".into()))?;
			if HDR_LEN + 32 + ct.len() + 2 + ct_e.len() > super::MAX_NOISE_MSG_LEN { return Err(Error::Protocol("noise msg1 too long".into())); }
		...
		if msg1.len() > super::MAX_NOISE_MSG_LEN { return Err(Error::Protocol("noise msg1 too long".into())); }
	...
	pub fn responder_handshake(r_static: &StaticKeypair, i_static_pk_expected: &[u8;32], msg1: &[u8], prologue: &[u8]) -> Result<ResponderResult> {
		if msg1.len() < MSG1_LEN_TOTAL { return Err(Error::Protocol("noise msg1 invalid length".into())); }
		let mut idx = 0usize;
		let mut has_hdr = false;
		let mut hdr_flags = 0u8;
		if msg1.len() >= HDR_LEN && msg1[0..2] == HDR_MAGIC {
			if msg1[2] != HDR_VER { return Err(Error::Protocol("noise header version".into())); }
			let kind_flags = msg1[3];
			if (kind_flags & 0xF0) != HDR_KIND_MSG1 { return Err(Error::Protocol("noise header type".into())); }
			if (kind_flags & HDR_FLAG_ROLE_I) == 0 { return Err(Error::Protocol("noise header role".into())); }
			has_hdr = true;
		...
		if ct.len() != MSG1_LEN_CIPHERTEXT { return Err(Error::Protocol("noise msg1 ciphertext length".into())); }
		...
		ss.mix_hash(ct);
		if s_i_pk.as_slice() != i_static_pk_expected { return Err(Error::Protocol("initiator static mismatch".into())); }
		...
		if msg1.len() > idx {
			...
			if !has_hdr { return Err(Error::Protocol("noise msg1 legacy early not allowed".into())); }
			if has_hdr && (hdr_flags & HDR_FLAG_0RTT) == 0 { return Err(Error::Protocol("noise msg1 unexpected tail".into())); }
			if msg1.len() < idx + 2 { return Err(Error::Protocol("noise msg1 early length field missing".into())); }
			let len = u16::from_be_bytes([msg1[idx], msg1[idx+1]]) as usize;
			idx += 2;
			if msg1.len() != idx + len { return Err(Error::Protocol("noise msg1 early length mismatch".into())); }
			let ct_e = &msg1[idx..idx+len];
			if ct_e.len() < 16 { return Err(Error::Protocol("noise msg1 early ciphertext too short".into())); }
			let mut out = [0u8; 32 + 12];
			ss.expand_ck(LBL_EARLY, &mut out);
			let mut k_e = [0u8;32]; k_e.copy_from_slice(&out[..32]);
			let mut n_e = [0u8;12]; n_e.copy_from_slice(&out[32..]);
		...
		Ok(ResponderResult { tx, rx, msg2, early_data: early_plain })
	}
	...
	pub fn initiator_verify_msg2(init: &mut InitiatorResult, msg2: &[u8]) -> Result<()> {
		...
		let aad2: [u8;32] = {
			...
			let mut d = sha2::Sha256::new(); d.update(init.handshake_hash); d.update(LBL_MSG2_AAD); d.finalize().into()
		};
		// Accept header if present; otherwise treat entire msg2 as ciphertext (legacy)
		let ct = if msg2.len() >= HDR_LEN && msg2[0..2] == HDR_MAGIC {
			if msg2[2] != HDR_VER { return Err(Error::Protocol("noise msg2 header".into())); }
			if (msg2[3] & 0xF0) != HDR_KIND_MSG2 { return Err(Error::Protocol("noise msg2 header".into())); }
			if (msg2[3] & HDR_FLAG_ROLE_R) == 0 { return Err(Error::Protocol("noise msg2 header role".into())); }
			&msg2[HDR_LEN..]
		} else { msg2 };
		let pt = cipher.open(AeadNonce([0u8;12]), &aad2, ct)?;
		if pt.as_slice() != MSG2_ACK { return Err(Error::Protocol("noise msg2 invalid".into())); }
		// ハンドシェイクハッシュもゼロ化
...
mod tests {
	...
	fn ik_demo_handshake_roundtrip() {
		...
		let exp_len = if &init.msg1[0..2] == b"NX" { MSG1_LEN_TOTAL + 4 } else { MSG1_LEN_TOTAL };
	...
	fn ik_demo_0rtt_roundtrip() {
		...
		let prologue = b"nyx-noise-lite";
		let eph = [9u8;32];
		let mut init = initiator_handshake_with_eph_seed_0rtt(&i, &r.pk, prologue, eph, Some(b"early-data")).unwrap();
	...
	fn ik_demo_0rtt_tamper_detected() {
		...
		let prologue = b"nyx-noise-lite";
		let eph = [10u8;32];
		let init = initiator_handshake_with_eph_seed_0rtt(&i, &r.pk, prologue, eph, Some(b"ED")).unwrap();
	...
	fn ik_demo_0rtt_legacy_without_header_rejected() {
		...
		let prologue = b"nyx-noise-lite";
		let eph = [12u8;32];
		let init = initiator_handshake_with_eph_seed_0rtt(&i, &r.pk, prologue, eph, Some(b"E")).unwrap();
	...
	fn ik_demo_rejects_wrong_initiator_pk() {
		...
		match res { Err(Error::Protocol(s)) => assert!(s.contains("mismatch")), _ => panic!("expected mismatch") }
	}
