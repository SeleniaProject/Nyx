...
// Global default for PQ mode used by handshake spans: 0=hybrid, 1=pq-only
static DEFAULT_PQ_MODE: AtomicU16 = AtomicU16::new(0);

/// Set default PQ mode for handshake spans (0=hybrid, 1=pq-only).
pub fn set_default_pq_mode(mode: u16) {
    DEFAULT_PQ_MODE.store(mode.min(1), Ordering::Relaxed);
}

/// Get default PQ mode for handshake spans.
pub fn get_default_pq_mode() -> u16 {
    DEFAULT_PQ_MODE.load(Ordering::Relaxed)
}

#[inline]
fn create_handshake_span() -> Option<Span> {
    // CID は上位層で生成されるためここでは unknown を設定（相関枠だけ確保）
    Some(tracing::span!(
        Level::INFO,
        "nyx.handshake",
        pq_mode = get_default_pq_mode(),
        cid = "unknown"
    ))
}
...
pub struct NoiseHandshake {
    ...
    // Tracing span to measure handshake lifecycle (classic mode)
    handshake_span: Option<Span>,
}
...
pub struct HybridNoiseHandshake {
    ...
    // Tracing span to measure handshake lifecycle (hybrid/PQ modes)
    handshake_span: Option<Span>,
...
impl NoiseHandshake {
    ...
    fn new_with_role(role: Role) -> Result<Self, NoiseError> {
        ...
        Ok(Self {
            ...
            chaining_key,
            symmetric_key: None,
            // Create handshake span for classic mode using negotiated default PQ_MODE (0=hybrid,1=pq-only)
            handshake_span: create_handshake_span(),
        })
    }
    ...
    fn write_initiator_first_message(
        &mut self,
        payload: &[u8],
        message: &mut [u8],
    ) -> Result<usize, NoiseError> {
    ...
    fn read_initiator_first_message(
        &mut self,
        message: &[u8],
        payload: &mut [u8],
    ) -> Result<usize, NoiseError> {
        ...
        // Telemetry: count early data acceptance on handshake payload reception
        if payload_len > 0 {
    ...
    fn write_responder_second_message(
        &mut self,
        payload: &[u8],
        message: &mut [u8],
    ) -> Result<usize, NoiseError> {
        ...
        let static_pub = match self.local_static.as_ref() {
            ...
            None => {
                return Err(NoiseError::InvalidState {
                    expected: "local static key".to_string(),
    ...
    fn read_responder_second_message(
        &mut self,
        message: &[u8],
        payload: &mut [u8],
    ) -> Result<usize, NoiseError> {
    ...
    fn write_initiator_third_message(
        &mut self,
        payload: &[u8],
        message: &mut [u8],
    ) -> Result<usize, NoiseError> {
        ...
        let static_pub = match self.local_static.as_ref() {
            ...
            None => {
                return Err(NoiseError::InvalidState {
                    expected: "local static key for message 2".to_string(),
                    actual: "None".to_string(),
                })
            }
        };
        ...
        self.state = HandshakeState::Completed;
        // Close handshake span on completion
    ...
    fn read_initiator_third_message(
        &mut self,
        message: &[u8],
        payload: &mut [u8],
    ) -> Result<usize, NoiseError> {
...
pub mod kyber {
    ...
    pub fn responder_keypair() -> Result<(PublicKey, SecretKey), crate::noise::NoiseError> {
        let mut rng = OsRng;
        let keypair = pqc_kyber::keypair(&mut rng).map_err(|_| {
            crate::noise::NoiseError::KeyGenerationFailed(
                "Kyber keypair generation failed".to_string(),
            )
        })?;
    ...
    pub fn initiator_encapsulate(
        pk: &PublicKey,
    ) -> Result<(Ciphertext, super::SessionKey), crate::noise::NoiseError> {
        let mut rng = OsRng;
        let (ciphertext, shared_secret) = pqc_kyber::encapsulate(pk, &mut rng).map_err(|_| {
            crate::noise::NoiseError::EncryptionFailed("Kyber encapsulation failed".to_string())
        })?;
    ...
    pub fn responder_decapsulate(
        ct: &Ciphertext,
        sk: &SecretKey,
    ) -> Result<super::SessionKey, crate::noise::NoiseError> {
        let shared_secret = pqc_kyber::decapsulate(ct, sk).map_err(|_| {
            crate::noise::NoiseError::DecryptionFailed("Kyber decapsulation failed".to_string())
        })?;
        let mut shared = [0u8; 32];
        shared.copy_from_slice(&shared_secret[..32]);
        let shared_secret = SharedSecret(shared);
...
pub mod hybrid {
    ...
    pub fn initiator_step(
        pk_kyber: &kyber::PublicKey,
    ) -> Result<(PublicKey, EphemeralSecret, kyber::Ciphertext, SessionKey), NoiseError> {
    ...
    pub fn responder_step(
        init_pub: &PublicKey,
        ct: &kyber::Ciphertext,
        sk_kyber: &kyber::SecretKey,
    ) -> Result<(PublicKey, SessionKey), NoiseError> {
        // Kyber part
    ...
    pub fn initiator_finalize(
        sec: EphemeralSecret,
        resp_pub: &PublicKey,
        kyber_key: SessionKey,
    ) -> Result<SessionKey, NoiseError> {
...
impl HybridNoiseHandshake {
    ...
    fn new_hybrid_role(role: Role, pq_algorithm: PqAlgorithm) -> Result<Self, NoiseError> {
        ...
        if matches!(pq_algorithm, PqAlgorithm::Bike) {
            return Err(NoiseError::HybridFailed(
                "BIKE algorithm is policy-disabled (unsupported)".into(),
            ));
        }

        // Generate hybrid keypair (X25519 + PQ)
        let (_local_public, local_secret) =
            generate_keypair(pq_algorithm).map_err(|e| NoiseError::HybridFailed(e.to_string()))?;
        ...
        Ok(Self {
            ...
            send_key: None,
            recv_key: None,
            // Create handshake span for hybrid mode using negotiated default PQ_MODE (0=hybrid,1=pq-only)
            handshake_span: create_handshake_span(),
        })
    }
    ...
    pub fn perform_ee_kyber_handshake(&mut self) -> Result<(SessionKey, SessionKey), NoiseError> {
        ...
        let _local_pk = ee_ext
            .generate_local_keypair(self.pq_algorithm)
    ...
    #[cfg(feature = "hybrid")]
    pub fn perform_se_kyber_handshake(
        &mut self,
        static_pk: &HybridPublicKey,
        payload: &[u8],
    ) -> Result<SessionKey, NoiseError> {
    ...
    fn write_hybrid_initiator_first_message(
        &mut self,
        payload: &[u8],
        message: &mut [u8],
    ) -> Result<usize, NoiseError> {
        #[cfg(feature = "hybrid")]
        {
            ...
            if self.remote_hybrid.is_none() {
                return Err(NoiseError::InvalidState {
                    expected: "remote hybrid public key preset".into(),
                    actual: "None".into(),
                });
            }
            let remote_pq_pk = match &self.remote_hybrid {
                Some(pk) => pk,
                None => unreachable!(),
            };
            ...
            let kyber_pk_bytes = match remote_pq_pk.kyber_public_bytes() {
                ...
                None => {
                    return Err(NoiseError::HybridFailed(
                        "Missing Kyber public key in remote hybrid key".into(),
                    ))
                }
            };
            ...
            let kyber_pk = {
                use pqc_kyber::*;
                pqc_kyber::PublicKey::from(*kyber_pk_bytes)
            };
    ...
    fn read_hybrid_responder_first_message(
        &mut self,
        message: &[u8],
        payload: &mut [u8],
    ) -> Result<usize, NoiseError> {
        #[cfg(feature = "hybrid")]
        {
            ...
            eph_bytes.copy_from_slice(&message[..32]);
            let remote_ephemeral = PublicKey::from(eph_bytes);
            let mut ct = [0u8; 1088];
            ct.copy_from_slice(&message[32..32 + 1088]);
            let remaining = &message[32 + 1088..];
            if payload.len() < remaining.len() {
                return Err(NoiseError::MessageTooShort {
            ...
            let local_sk = match &self.local_hybrid {
                Some(s) => s,
                None => {
                    return Err(NoiseError::HybridFailed(
                        "local hybrid secret missing".into(),
                    ))
                }
            };
            let kyber_sk_bytes = match local_sk.kyber_secret_bytes() {
                Some(s) => s,
                None => {
                    return Err(NoiseError::HybridFailed(
                        "local Kyber secret missing".into(),
                    ))
                }
            };
            #[cfg(feature = "kyber")]
    ...
    fn write_hybrid_responder_second_message(
        &mut self,
        payload: &[u8],
        message: &mut [u8],
    ) -> Result<usize, NoiseError> {
        #[cfg(feature = "hybrid")]
        {
            ...
            let dh_key = {
                let r_pub = self
                    .remote_ephemeral
                    .ok_or_else(|| NoiseError::HybridFailed("missing remote eph".into()))?;
                let l_sec = self
                    .local_ephemeral
                    .take()
                    .ok_or_else(|| NoiseError::HybridFailed("missing local eph".into()))?;
                l_sec.diffie_hellman(&r_pub)
            };
            let kyber_part = self
                .kyber_partial
                .as_ref()
                .ok_or_else(|| NoiseError::HybridFailed("Kyber partial missing".into()))?;
            ...
            let send_key = self
                .send_key
                .as_ref()
                .ok_or_else(|| NoiseError::HybridFailed("missing send key".into()))?;
    ...
    fn read_hybrid_initiator_second_message(
        &mut self,
        message: &[u8],
        payload: &mut [u8],
    ) -> Result<usize, NoiseError> {
        #[cfg(feature = "hybrid")]
        {
            ...
            let mut eph = [0u8; 32];
            eph.copy_from_slice(&message[..32]);
            ...
            let recv_key = self
                .recv_key
                .as_ref()
                .ok_or_else(|| NoiseError::HybridFailed("missing recv key".into()))?;
            let aead = NyxAead::new(recv_key);
            let nonce = [0u8; 12];
            let pt = aead.decrypt(&nonce, ct, &[])?;
            if pt.len() > payload.len() {
                return Err(NoiseError::MessageTooShort {
                    expected: pt.len(),
                    actual: payload.len(),
                });
    ...
    fn write_hybrid_initiator_third_message(
        &mut self,
        payload: &[u8],
        message: &mut [u8],
    ) -> Result<usize, NoiseError> {
        ...
        let aead = NyxAead::new(send_key);
        let nonce = [0u8; 12];
        let ct = aead.encrypt(&nonce, payload, &[])?;
        let total = 2 + ct.len();
    ...
    fn read_hybrid_responder_third_message(
        &mut self,
        message: &[u8],
        payload: &mut [u8],
    ) -> Result<usize, NoiseError> {
    ...
    pub fn into_hybrid_transport_mode(self) -> Result<NoiseTransport, NoiseError> {
        ...
        let send = self
            .send_key
            .clone()
            .or(self.symmetric_key.clone())
            .unwrap_or(SessionKey([0u8; 32]));
        let recv = self
            .recv_key
            .clone()
            .or(self.symmetric_key.clone())
            .unwrap_or(send.clone());
...
impl HybridNoiseHandshake {
    ...
    fn derive_directional_keys(&mut self) {
        if self.send_key.is_some() && self.recv_key.is_some() {
            return;
        }
        ...
        match self.role {
            Role::Initiator => {
                self.send_key = Some(SessionKey(k1));
                self.recv_key = Some(SessionKey(k2));
            }
            Role::Responder => {
                self.send_key = Some(SessionKey(k2));
                self.recv_key = Some(SessionKey(k1));
            }
        }
...
mod hybrid_tests {
    ...
    fn test_simple_hybrid_flow() {
        ...
        let mut initiator =
            HybridNoiseHandshake::new_hybrid_initiator(PqAlgorithm::Kyber1024).unwrap();
        let mut responder =
            HybridNoiseHandshake::new_hybrid_responder(PqAlgorithm::Kyber1024).unwrap();
        ...
        let p1 = b"hello";
        let l1 = initiator.write_hybrid_message(p1, &mut m1).unwrap();
        m1.truncate(l1);
        // responder read
        let mut buf = vec![0u8; 64];
        let _ = responder.read_hybrid_message(&m1, &mut buf).unwrap();
        // responder second
        let mut m2 = vec![0u8; 32 + 2 + 64];
        let p2 = b"ok";
        let l2 = responder.write_hybrid_message(p2, &mut m2).unwrap();
        m2.truncate(l2);
        // initiator read
        let mut buf2 = vec![0u8; 32];
        let _ = initiator.read_hybrid_message(&m2, &mut buf2).unwrap();
        // initiator third (optional ack)
        let mut m3 = vec![0u8; 2 + 64];
        let p3 = b"!";
        let l3 = initiator.write_hybrid_message(p3, &mut m3).unwrap();
        m3.truncate(l3);
        let mut buf3 = vec![0u8; 8];
        let _ = responder.read_hybrid_message(&m3, &mut buf3).unwrap();
        ...
        assert_eq!(
            initiator.send_key.as_ref().unwrap(),
            responder.recv_key.as_ref().unwrap()
        );
        assert_ne!(
            initiator.send_key.as_ref().unwrap().0,
            initiator.recv_key.as_ref().unwrap().0
        );
    }
    ...
    fn test_hybrid_message_too_short() {
        ...
        let err = responder
            .read_hybrid_message(&[], &mut payload_buf)
            .unwrap_err();
        match err {
            NoiseError::MessageTooShort { .. } => {}
            _ => panic!("expected MessageTooShort"),
        }
    }
    ...
    fn test_hybrid_replay_second_message() {
        ...
        let l1 = initiator.write_hybrid_message(b"hi", &mut m1).unwrap();
        m1.truncate(l1);
        let mut tmp = vec![0u8; 32];
        responder.read_hybrid_message(&m1, &mut tmp).unwrap();
        let mut m2 = vec![0u8; 32 + 2 + 32];
        let l2 = responder.write_hybrid_message(b"ok", &mut m2).unwrap();
        m2.truncate(l2);
        let mut tmp2 = vec![0u8; 32];
        initiator.read_hybrid_message(&m2, &mut tmp2).unwrap();
        let mut tmp3 = vec![0u8; 32];
        let err = initiator.read_hybrid_message(&m2, &mut tmp3).unwrap_err();
        match err {
            NoiseError::InvalidState { .. } => {}
            _ => panic!("expected InvalidState on replay"),
        }
    }
    ...
    fn test_hybrid_corrupted_kyber_ct() {
        ...
        if let Err(e) = result {
            match e {
                NoiseError::DecryptionFailed(_) | NoiseError::HybridFailed(_) => {}
                _ => panic!("unexpected error kind {e:?}"),
            }
        } else {
            // 成功した場合: 取得した kyber_partial が同じ条件の正常ハンドシェイクと比べて異なるはず
            // 正常ハンドシェイクを再現し比較
            let mut responder_ref =
                HybridNoiseHandshake::new_hybrid_responder(PqAlgorithm::Kyber1024).unwrap();
            let mut initiator_ref =
                HybridNoiseHandshake::new_hybrid_initiator(PqAlgorithm::Kyber1024).unwrap();
            let resp_pub_ref = responder_ref
                .local_hybrid
                .as_ref()
                .unwrap()
                .public()
            ...
            let l1r = initiator_ref
                .write_hybrid_message(b"hello", &mut m1_ref)
                .unwrap();
            m1_ref.truncate(l1r);
            let mut buf_ref = vec![0u8; 64];
            responder_ref
                .read_hybrid_message(&m1_ref, &mut buf_ref)
                .unwrap();
            ...
            assert_ne!(
                corrupt_key.unwrap(),
                ref_key.unwrap(),
                "corrupted CT produced same Kyber partial key (unexpected)"
            );
        }
    }
    ...
    fn test_hybrid_corrupted_kyber_ct_stability() {
        ...
        let l = initiator_ref
            .write_hybrid_message(b"ok", &mut base)
            .unwrap();
        base.truncate(l);
        let mut buf_ref = vec![0u8; 32];
        responder_ref
            .read_hybrid_message(&base, &mut buf_ref)
            .unwrap();
        ...
        for _ in 0..16 {
            ...
            // ランダムに 6 箇所 flip
            for _i in 0..6 {
                let idx = 32 + rng.gen_range(0..1088);
                mutated[idx] ^= 1u8 << (rng.gen_range(0..8));
            }
            ...
            match r.read_hybrid_message(&mutated, &mut buf) {
                ...
                Ok(_) => {
                    if let Some(partial) = r.kyber_partial.as_ref() {
                        if partial.0 != reference {
                            observed_divergence = true;
                        }
                    }
                }
            }
            if observed_error || observed_divergence {
                break;
            }
        }
        assert!(
            observed_error || observed_divergence,
            "no divergence or error observed after multiple corruptions"
        );
    }
    ...
    fn test_hybrid_forward_secrecy_ephemeral_cleared() {
        ...
        initiator.remote_hybrid = Some(resp_pub);
        // M1
        let mut m1 = vec![0u8; 32 + 1088 + 2];
        let l1 = initiator.write_hybrid_message(b"x", &mut m1).unwrap();
        m1.truncate(l1);
        let mut tmp = vec![0u8; 16];
        responder.read_hybrid_message(&m1, &mut tmp).unwrap();
        // M2
        let mut m2 = vec![0u8; 32 + 2 + 64];
        let l2 = responder.write_hybrid_message(b"y", &mut m2).unwrap();
        m2.truncate(l2);
        let mut tmp2 = vec![0u8; 16];
        initiator.read_hybrid_message(&m2, &mut tmp2).unwrap();
        // M3
        let mut m3 = vec![0u8; 2 + 64];
        let l3 = initiator.write_hybrid_message(b"!", &mut m3).unwrap();
        m3.truncate(l3);
        let mut tmp3 = vec![0u8; 8];
        responder.read_hybrid_message(&m3, &mut tmp3).unwrap();
        assert!(
            initiator.local_ephemeral.is_none(),
            "initiator ephemeral not cleared"
        );
        assert!(
            responder.local_ephemeral.is_none(),
            "responder ephemeral not cleared"
        );
    }
    ...
    fn test_hybrid_forward_secrecy_multi_handshake_uniqueness() {
        ...
        for _ in 0..N {
            ...
            assert!(
                send_keys.insert(isk),
                "duplicate send key observed across independent handshakes"
            );
            assert!(
                recv_keys.insert(irk),
                "duplicate recv key observed across independent handshakes"
            );
        }
