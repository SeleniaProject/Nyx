...
use hpke::kem::Kem;
use hpke::{aead::ChaCha20Poly1305, kdf::HkdfSha256, kem::X25519HkdfSha256, OpModeR, OpModeS};
...
pub fn generate_and_seal_session(
    pk_r: &PublicKey,
    info: &[u8],
    aad: &[u8],
) -> Result<(Vec<u8>, Vec<u8>, SessionKey), hpke::HpkeError> {
    let mut secret = [0u8; 32];
    OsRng.fill_bytes(&mut secret);
    let (enc, mut sender_ctx) =
        hpke::setup_sender::<Aead, Kdf, KemImpl, _>(&OpModeS::Base, pk_r, info, &mut OsRng)?;
...
pub fn open_session(
    ...
    aad: &[u8],
    ciphertext: &[u8],
) -> Result<SessionKey, hpke::HpkeError> {
    let pt = open(sk_r, enc, info, aad, ciphertext)?;
    if pt.len() != 32 {
        return Err(hpke::HpkeError::OpenError);
    }
    let mut arr = [0u8; 32];
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self, CryptoError> {
        if data.is_empty() {
            return Err(CryptoError::DeserializationError(
                "Empty encapsulated key data".to_string(),
            ));
        }
        Ok(Self {
            inner: data.to_vec(),
...
impl HpkeKeyDeriver {
    ...
    pub fn derive_keypair_with_retry(
        &self,
        max_retries: u32,
    ...
    pub fn derive_keypair_with_rng<R: RngCore + CryptoRng>(
        &self,
        rng: &mut R,
    ) -> Result<(PrivateKey, PublicKey), CryptoError> {
    ...
    fn validate_generated_keypair(
        ...
        match self.encapsulate(public_key) {
            Ok(_) => Ok(()),
            Err(e) => Err(CryptoError::KeyValidationFailed(format!(
                "Key failed encapsulation test: {:?}",
                e
            ))),
        }
    }
    ...
    pub fn encapsulate_with_rng<R: RngCore + CryptoRng>(
        ...
        let (shared_secret, encapped_key) =
            KemImpl::encap(pk, None, rng).map_err(|e| CryptoError::InvalidPublicKey)?;

        // Validate shared secret
        if shared_secret.0.is_empty() {
            return Err(CryptoError::KeyDerivationFailed(
                "Empty shared secret generated".to_string(),
            ));
        }
    ...
    pub fn decapsulate(
        ...
        let encapped = <KemImpl as Kem>::EncappedKey::from_bytes(&enc.inner).map_err(|e| {
            CryptoError::DeserializationError(format!("Failed to parse encapsulated key: {:?}", e))
        })?;

        // Perform decapsulation
        let shared_secret =
            KemImpl::decap(sk, None, &encapped).map_err(|e| CryptoError::InvalidEncapsulatedKey)?;

        // Validate shared secret
        if shared_secret.0.is_empty() {
            return Err(CryptoError::KeyDerivationFailed(
                "Empty shared secret from decapsulation".to_string(),
            ));
        }
    ...
    fn validate_public_key_format(&self, pk: &PublicKey) -> Result<(), CryptoError> {
        ...
        if key_bytes.iter().all(|&x| x == 0) {
            return Err(CryptoError::KeyValidationFailed(
                "Public key is all zeros".to_string(),
            ));
        }
    ...
    fn validate_private_key_format(&self, sk: &PrivateKey) -> Result<(), CryptoError> {
        ...
        if key_bytes.iter().all(|&x| x == 0) {
            return Err(CryptoError::KeyValidationFailed(
                "Private key is all zeros".to_string(),
            ));
        }
    ...
    fn validate_encapsulated_key_format(&self, enc: &EncapsulatedKey) -> Result<(), CryptoError> {
        ...
        if enc.inner.iter().all(|&x| x == 0) {
            return Err(CryptoError::KeyValidationFailed(
                "Encapsulated key is all zeros".to_string(),
            ));
        }
    ...
    pub fn derive_session_key(
        ...
        if shared_secret.len() == 0 {
            return Err(CryptoError::KeyDerivationFailed(
                "Empty shared secret".to_string(),
            ));
        }

        // Validate context
        if context.is_empty() {
            return Err(CryptoError::ContextError(
                "Empty context not allowed".to_string(),
            ));
        }

        // Derive key material
        let key_material = hkdf_expand(shared_secret.as_bytes(), KdfLabel::Custom(context), 32);

        // Validate derived key material
        if key_material.iter().all(|&x| x == 0) {
            return Err(CryptoError::KeyDerivationFailed(
                "Derived key is all zeros".to_string(),
            ));
        }
    ...
    pub fn derive_multiple_session_keys(
        &self,
        shared_secret: &SharedSecret,
        contexts: &[&'static [u8]],
    ) -> Result<Vec<SessionKey>, CryptoError> {
            ...
            Err(e) => {
                if let Ok(mut stats) = self.stats.lock() {
                    match &e {
                        ...
                        CryptoError::KeyValidationFailed(_) => {
...
pub fn open(
    ...
    let mut receiver_ctx =
        hpke::setup_receiver::<Aead, Kdf, KemImpl>(&OpModeR::Base, sk_r, &encapped, info)?;
...
mod tests {
    ...
    fn hpke_key_deriver_session_key_derivation() {
        ...
        let session_key1 = deriver
            .derive_session_key(&shared_secret, b"context1")
            .unwrap();
        let session_key2 = deriver
            .derive_session_key(&shared_secret, b"context2")
            .unwrap();
        let session_key3 = deriver
            .derive_session_key(&shared_secret, b"context1")
            .unwrap();
    ...
    fn hpke_key_deriver_enhanced_session_key_derivation() {
        ...
        let session_key = deriver
            .derive_session_key(&shared_secret, b"test-context")
            .unwrap();
        ...
        let session_keys = deriver
            .derive_multiple_session_keys(&shared_secret, contexts)
            .unwrap();
    ...
    fn hpke_monitored_key_deriver() {
        ...
        let _session_key = monitored
            .derive_session_key(&shared_secret, b"test")
            .unwrap();
        let stats = monitored.get_stats().unwrap();
    ...
    fn hpke_shared_secret_validation() {
        ...
        let session_key = deriver
            .derive_session_key(&shared_secret, b"valid-context")
            .unwrap();
        assert_ne!(session_key.0, [0u8; 32]);
    }
