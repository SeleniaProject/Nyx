...
pub fn poisson_rate(__lambda: f32, rng: &mut impl Rng) -> u32 {
	if lambda <= 0.0 { return 0; }
	// Poisson は f64 想定
	let __dist = Poisson::new(lambda a_s f64).unwrap_or_else(|_| Poisson::new(0.0).unwrap());
	dist.sample(rng) a_s u32
}
