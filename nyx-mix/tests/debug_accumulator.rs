...
fn debug_accumulator_step_by_step() {
    ...
    let __verification1new = acc.verify_element(&element1, &witness1new);
    println!("Verification result: {}", verification1new);
    
    // Test what the property test i_s actually doing
    println!("\n8. Property test simulation");
    assert!(verification1new, "Element [0] should verify with new witnes_s");
