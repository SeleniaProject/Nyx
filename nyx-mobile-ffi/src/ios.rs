...
#[cfg(target_os = "ios")]
use std::os::raw::{c_char as _c_char, c_float, c_int};
#[cfg(target_os = "ios")]
...
pub extern "C" fn ios_get_battery_level() -> c_float {
    unsafe {
        ...
        let battery_monitoring_enabled: bool =
            msg_send![current_device, isBatteryMonitoringEnabled];
...
pub extern "C" fn ios_is_charging() -> c_int {
    unsafe {
        ...
        debug!(
            "iOS charging state: {} (raw: {})",
            is_charging, battery_state
        );
...
pub extern "C" fn ios_register_battery_notifications() -> c_int {
    unsafe {
        ...
        let battery_level_notification =
            NSString::alloc(cocoa::base::nil).init_str("UIDeviceBatteryLevelDidChangeNotification");

        let battery_state_notification =
            NSString::alloc(cocoa::base::nil).init_str("UIDeviceBatteryStateDidChangeNotification");
...
pub extern "C" fn ios_register_app_notifications() -> c_int {
    unsafe {
        ...
        let did_become_active =
            NSString::alloc(cocoa::base::nil).init_str("UIApplicationDidBecomeActiveNotification");

        let will_resign_active =
            NSString::alloc(cocoa::base::nil).init_str("UIApplicationWillResignActiveNotification");
