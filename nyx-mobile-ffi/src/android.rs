...
pub extern "C" fn android_get_battery_level() -> c_int {
    match get_jni_env() {
        Ok(env) => match get_battery_level_impl(&env) {
            Ok(level) => {
...
pub extern "C" fn android_is_charging() -> c_int {
    match get_jni_env() {
        Ok(env) => match is_charging_impl(&env) {
            Ok(charging) => {
...
pub extern "C" fn android_is_screen_on() -> c_int {
    match get_jni_env() {
        Ok(env) => match is_screen_on_impl(&env) {
            Ok(screen_on) => {
...
fn is_screen_on_impl(env: &JNIEnv) -> Result<bool, Box<dyn std::error::Error + Send + Sync>> {
    ...
    let is_interactive = env.call_method(power_manager.l()?, "isInteractive", "()Z", &[])?;
...
pub extern "C" fn android_is_power_save_mode() -> c_int {
    match get_jni_env() {
        Ok(env) => match is_power_save_mode_impl(&env) {
            Ok(power_save) => {
...
fn is_power_save_mode_impl(env: &JNIEnv) -> Result<bool, Box<dyn std::error::Error + Send + Sync>> {
    ...
    let is_power_save = env.call_method(power_manager.l()?, "isPowerSaveMode", "()Z", &[])?;
...
pub extern "C" fn android_get_app_state() -> c_int {
    match get_jni_env() {
        Ok(env) => match get_app_state_impl(&env) {
            Ok(state) => {
...
pub extern "C" fn android_get_network_state() -> c_int {
    match get_jni_env() {
        Ok(env) => match get_network_state_impl(&env) {
            Ok(state) => {
...
fn get_network_state_impl(env: &JNIEnv) -> Result<c_int, Box<dyn std::error::Error + Send + Sync>> {
    ...
    // Get network type
    let network_type = env.call_method(active_network_info.l()?, "getType", "()I", &[])?;
...
#[cfg(not(target_os = "android"))]
#[no_mangle]
pub extern "C" fn android_init_jni(
