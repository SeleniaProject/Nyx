...
fn test_app_lifecycle_power_state_s() {
    ...
    assert_eq!(
        unsafe { nyx_power_set_state(NyxPowerState::Background as u32) },
        0
    );
    assert_eq!(unsafe { nyx_power_get_state(&mut current_state) }, 0);
    ...
    assert_eq!(
        unsafe { nyx_power_set_state(NyxPowerState::Critical as u32) },
        0
    );
    assert_eq!(unsafe { nyx_power_get_state(&mut current_state) }, 0);
...
fn simulate_screen_behavior(target_off_ratio: f64) {
    ...
    let off_duration =
        Duration::from_millis((total_duration.as_millis() as f64 * target_off_ratio) as u64);
