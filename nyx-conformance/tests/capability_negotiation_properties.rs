...
use nyx_stream::{
    decode_caps, encode_caps, negotiate, perform_cap_negotiation, Capability, NegotiationError,
    FLAG_REQUIRED, LOCAL_CAP_IDS,
};
...
fn capability_set_strategy() -> impl Strategy<Value = Vec<Capability>> {
    prop::collection::vec(capability_strategy(), 0..=16).prop_map(|mut caps| {
...
fn supported_caps_strategy() -> impl Strategy<Value = Vec<u32>> {
    prop::collection::vec(capability_id_strategy(), 1..=16).prop_map(|ids| {
...
mod deterministic_tests {
    ...
    fn local_capabilities_always_supported() {
        ...
        assert!(
            result.is_ok(),
            "Local capabilities should always be supported"
        );
    ...
    fn core_capability_required() {
        ...
        assert!(
            result_unsupported.is_err(),
            "Unsupported required capability should fail"
        );
    }
    ...
    fn capability_flags_interpretation() {
        ...
        let required_with_extra_flags = Capability {
            id: 0x0001,
            flags: FLAG_REQUIRED | 0x02,
            data: vec![],
        };
