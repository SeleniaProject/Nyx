...
use nyx_conformance::{
    ...
    property_tester::{ByteVecGenerator, PropertyTestConfig, PropertyTester},
};
...
async fn test_client_daemon_data_flow() {
    ...
    let (client_private, client_public) = client_hpke
        .derive_keypair()
        .expect("Failed to derive client keypair");
    let (daemon_private, daemon_public) = daemon_hpke
        .derive_keypair()
        .expect("Failed to derive daemon keypair");

    // Perform HPKE key exchange
    let (shared_secret, encapsulated_key) = client_hpke
        .encapsulate(&daemon_public)
        .expect("Failed to encapsulate key");
    let decapsulated_secret = daemon_hpke
        .decapsulate(&daemon_private, &encapsulated_key)
    ...
    for client_id in 0..num_clients {
        ...
        let handle = tokio::spawn(async move {
            ...
            let (private_key, public_key) =
                hpke.derive_keypair().expect("Failed to derive keypair");

            // Phase 2: Noise handshake simulation
            let mut noise =
                NoiseHandshake::new_initiator().expect("Failed to create noise handshake");
            ...
            info!(
                "Client {} using pattern: {} ({} messages of {} bytes)",
            ...
            info!(
                "Client {} completed: {} bytes sent/received in {:?}",
                client_name,
            ...
            Ok(client_result) => {
                if client_result.success {
                    ...
                    total_bytes_transferred +=
                        client_result.bytes_sent + client_result.bytes_received;
                    ...
                    warn!(
                        "Client {} failed: {:?}",
                        client_result.client_name, client_result.error
                    );
                }
            }
            Err(e) => {
...
async fn test_multipath_communication() {
    ...
    for (i, path_config) in path_configs.iter().enumerate() {
        ...
        context
            .network_simulator
            .add_node(node_id, Some((i as f64 * 10.0, i as f64 * 5.0)))
            .await;

        // Configure path-specific network conditions
        let path_simulator = &context.network_simulator;
        path_simulator
            .simulate_packet_loss(path_config.loss_rate)
            .await;
        path_simulator
            .simulate_latency(LatencyDistribution::Fixed {
                latency_ms: path_config.latency_ms,
            })
            .await;
        path_simulator
            .simulate_bandwidth_limit(path_config.bandwidth_mbps)
            .await;

        info!(
            "Configured path '{}': {}ms latency, {}Mbps bandwidth, {:.1}% loss",
            path_config.name,
            path_config.latency_ms,
            path_config.bandwidth_mbps,
    ...
    for (test_name, base_data, packet_count) in test_data_sets {
        info!(
            "Running multipath test: {} ({} packets of {} bytes)",
            test_name,
            packet_count,
            base_data.len()
        );
                    ...
                    if packet_result.success {
                        ...
                        *path_usage_stats.entry(packet_result.path_id).or_insert(0) += 1;
                        ...
                        if let Some(metrics) = path_metrics.get_mut(&packet_result.path_id) {
                            ...
                            metrics.avg_latency = (current_avg * (packet_count - 1) as u32
                                + packet_result.send_duration)
                                / packet_count as u32;
                        }
                    } else {
                        failed_packets += 1;
        ...
        let throughput_mbps =
            (test_bytes_sent * 8) as f64 / (test_duration.as_secs_f64() * 1_000_000.0);

        info!(
            "Test '{}' completed: {}/{} packets successful ({:.1}%), {:.2} Mbps throughput",
            test_name,
            successful_packets,
            packet_count,
            success_rate * 100.0,
    ...
    for (path_id, usage_count) in &path_usage_stats {
        ...
        info!(
            "  Path '{}': {} packets, {:.2} Mbps throughput, {:.1}% loss, {:?} avg latency",
            path_config.name,
            usage_count,
            throughput,
            loss_rate * 100.0,
    ...
    let successful_failovers = failover_results
        .iter()
        .filter(|r| r.as_ref().map_or(false, |res| res.is_ok()))
        .count();

    info!(
        "Failover test: {}/{} packets successfully rerouted",
    ...
    assert!(
        path_usage_stats.len() >= 3,
        "Should have used multiple paths"
    );
    assert!(
        successful_failovers >= failover_packets / 2,
        "Should have reasonable failover success"
    );
...
async fn test_long_running_connection() {
    ...
    let hpke = HpkeKeyDeriver::new().expect("Failed to create HPKE");
    let (private_key, public_key) = hpke.derive_keypair().expect("Failed to derive keypair");
    ...
    context.record_connection(conn_info).await;
    info!(
        "Long-running connection established for client: {}",
        client_name
    );
    ...
    for pattern in activity_patterns {
        ...
        let handle = tokio::spawn(async move {
                ...
                if Instant::now() >= next_send {
                    ...
                    match context_clone.network_simulator.send_packet(packet).await {
                        Ok(()) => {
                            ...
                            pattern_stats.avg_latency = (pattern_stats.avg_latency
                                * (total_sends - 1) as u32
                                + send_latency)
                                / total_sends as u32;

                            // Record data transfer
                            context_clone
                                .record_data_transfer(connection_id, message_data.len() as u64, 0)
                                .await;
                        }
                        Err(e) => {
            ...
            info!(
                "Pattern '{}' completed: {} messages, {} bytes, {:.1}% success rate",
                pattern_name,
                pattern_stats.messages_sent,
                pattern_stats.bytes_sent,
                (pattern_stats.successful_sends as f64 / pattern_stats.messages_sent as f64)
                    * 100.0
            );
        ...
        tokio::spawn(async move {
            ...
            while Instant::now() < end_time {
                ...
                if let Some(conn) = connections.get(&connection_id) {
                    ...
                    if !is_healthy {
                        warn!(
                            "Connection health degraded: {} since last activity",
    ...
    for (pattern_name, handle) in pattern_handles {
        match handle.await {
            Ok(stats) => {
                ...
                info!(
                    "Pattern '{}' final stats: {} msgs, {} bytes, {:?} avg latency",
    ...
    let health_samples = health_monitor_handle
        .await
        .expect("Health monitor should complete");
    ...
    info!(
        "Connection health: {:.1}% healthy samples ({}/{})",
        health_percentage,
        healthy_samples,
        health_samples.len()
    );
    ...
    let resilience_success = resilience_results
        .iter()
        .filter(|r| r.as_ref().map_or(false, |res| res.is_ok()))
        .count();
    ...
    info!(
        "Resilience test: {:.1}% success rate under stress ({}/{})",
    ...
    let connection_duration = final_metrics
        .test_start_time
    ...
    info!(
        "  Success rate: {:.1}%",
        (total_successful as f64 / total_messages as f64) * 100.0
    );
    ...
    assert!(
        ...
        "Should maintain good connection health"
    );
    assert!(
        resilience_rate > 50.0,
        "Should show reasonable resilience under stress"
    );
...
async fn test_stress_scenario() {
    ...
    info!(
        "Starting stress test: {} connections, {} messages each",
        ...
        let handle = tokio::spawn(async move {
            ...
            let hpke = match hpke_result {
                ...
                Err(e) => {
                    stress_result
                        .errors
                        .push(format!("HPKE creation failed: {:?}", e));
            ...
            let (private_key, public_key) = match keypair_result {
                ...
                Err(e) => {
                    stress_result
                        .errors
                        .push(format!("Keypair derivation failed: {:?}", e));
                    results_clone.lock().unwrap().push(stress_result);
                    return;
                }
            };
            ...
            for msg_id in 0..messages_per_connection {
                ...
                match context_clone.network_simulator.send_packet(packet).await {
                    ...
                    Err(e) => {
                        stress_result
                            .errors
                            .push(format!("Message {} send failed: {:?}", msg_id, e));
                    }
                }
            ...
            stress_result.success =
                success_rate >= 0.7 && stress_result.establishment_time < Duration::from_secs(1);
            ...
            if stress_result.success {
                ...
                context_clone
                    .update_connection_status(
                        connection_id,
                        ConnectionStatus::Failed("Stress test criteria not met".to_string()),
                    )
                    .await;
            }
    ...
    info!(
        "Waiting for {} stress test connections to complete",
        connection_count
    );
    ...
    info!(
        ...
        completed_connections, connection_count
    );
    info!(
    ...
    assert!(
        completed_connections >= (connection_count * 8 / 10) as usize,
    ...
    assert!(
        avg_establishment_time < Duration::from_secs(2),
...
async fn test_network_failure_recovery() {
    ...
    context
        .network_simulator
        .simulate_latency(LatencyDistribution::Fixed { latency_ms: 20.0 })
        .await;
    ...
    info!(
        "Baseline established: {:.1}% success, {:?} avg latency",
        baseline_stats.success_rate() * 100.0,
    ...
    for (loss_rate, latency_ms, phase_name) in degradation_steps {
        ...
        context
            .network_simulator
            .simulate_packet_loss(loss_rate)
            .await;
        context
            .network_simulator
            .simulate_latency(LatencyDistribution::Fixed { latency_ms })
            .await;
        ...
        info!(
            "Phase '{}': {:.1}% success, {:?} avg latency",
            phase_name,
            phase_stats.success_rate() * 100.0,
            phase_stats.avg_latency()
        );
    ...
    context
        .network_simulator
        .simulate_latency(LatencyDistribution::Fixed { latency_ms: 1000.0 })
        .await;
    ...
    for i in 0..failure_messages {
        ...
        match timeout(
            ...
            context.network_simulator.send_packet(packet),
        )
        .await
        {
            Ok(Ok(())) => {
    ...
    info!(
        "Complete failure phase: {:.1}% success (expected to be very low)",
    ...
    context
        .update_connection_status(
            connection_id,
            ConnectionStatus::Failed("Network failure detected".to_string()),
        )
        .await;
    ...
    for (loss_rate, latency_ms, phase_name) in recovery_steps {
        info!(
            "Recovery phase {}: {:.1}% loss, {}ms latency",
            phase_name,
            loss_rate * 100.0,
            latency_ms
        );
        ...
        info!(
            "Recovery phase '{}': {:.1}% success, {:?} avg latency",
            phase_name,
            phase_stats.success_rate() * 100.0,
            phase_stats.avg_latency()
        );
    ...
    info!(
        "Post-recovery validation: {:.1}% success, {:?} avg latency",
        validation_stats.success_rate() * 100.0,
        validation_stats.avg_latency()
    );
    ...
    info!(
        "  Post-recovery success rate: {:.1}%",
        final_success * 100.0
    );
    info!(
        "  Recovery effectiveness: {:.1}%",
        recovery_effectiveness * 100.0
    );
    info!(
        "  Total bytes transferred: {}",
        final_metrics.total_bytes_transferred
    );
    ...
    assert!(
        failure_stats.success_rate() < 0.3,
        "Failure phase should have low success rate"
    );
    assert!(
        final_success > 0.7,
        "Should recover to reasonable success rate"
    );
    assert!(
        recovery_effectiveness > 0.8,
        "Recovery should be at least 80% effective"
    );
    ...
    assert!(
        recovery_improving,
        "Recovery should show improving success rates"
    );
...
async fn test_data_integrity_pipeline() {
    ...
    let test_datasets = vec![
        (
            "text_data",
            b"Critical text data that must maintain integrity through the entire Nyx pipeline"
                .to_vec(),
        ),
        ...
        (
            "random_data",
            (0..4096).map(|_| thread_rng().gen::<u8>()).collect(),
        ),
        ("structured_data", create_structured_test_data()),
        (
            "unicode_data",
            "🔒 Encrypted data with émojis and spëcial chars 中文 العربية"
                .as_bytes()
                .to_vec(),
        ),
    ];
    ...
    for (data_name, original_data) in test_datasets {
        ...
        let (shared_secret, encapsulated_key) = hpke
            .encapsulate(&public_key)
            .expect("Failed to encapsulate");
        let decapsulated_secret = hpke
            .decapsulate(&private_key, &encapsulated_key)
        ...
        if integrity_test.overall_success {
            ...
            for phase_result in &integrity_test.phase_results {
                if !phase_result.integrity_maintained {
                    error!(
                        "  Phase '{}' failed: expected {} bytes, got {} bytes",
    ...
    let property_success_rate = property_results.iter().map(|r| r.success_rate).sum::<f64>()
        / property_results.len() as f64;
    info!(
        "  Property test success rate: {:.1}%",
        property_success_rate * 100.0
    );

    // Assertions for test success
    assert!(
        success_rate >= 0.9,
        "Should have at least 90% integrity success rate"
    );
    assert!(failed_tests == 0, "Should have no integrity failures");
    assert!(
        property_success_rate >= 0.95,
