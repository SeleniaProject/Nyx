...
async fn test_crypto_stream_integration() {
    ...
    let (private_key, public_key) = hpke_deriver
        .derive_keypair()
    ...
    let msg1_len = initiator
        .write_message(b"hello", &mut message_buffer)
...
async fn test_network_simulation_integration() {
    ...
    context
        .network_simulator
        .set_latency_distribution(Duration::from_millis(50), Duration::from_millis(10))
        .await;
    context
        .network_simulator
        .set_bandwidth_limit(1_000_000)
        .await; // 1 Mbps

    // Create simulated network path
    let path = context
        .network_simulator
        .create_path(vec!["node1", "node2", "node3"])
        .await
    ...
    match result {
        Ok(received_data) => {
            ...
            info!(
                "Data transmitted successfully: {} bytes in {:?}",
                received_data.len(),
                transmission_time
            );
        }
        Err(e) => {
            warn!(
                "Data transmission failed (expected with packet loss): {:?}",
                e
            );
        }
    }
    ...
    context
        .network_simulator
        .create_partition(vec!["node1"], vec!["node2", "node3"])
        .await;

    // Verify partition affects connectivity
    let partition_result = context
        .network_simulator
        .test_connectivity("node1", "node3")
        .await;
    assert!(
        !partition_result,
        "Connectivity should be blocked by partition"
    );
    ...
    let healed_result = context
        .network_simulator
        .test_connectivity("node1", "node3")
        .await;
    assert!(
        healed_result,
        "Connectivity should be restored after healing partition"
    );
...
async fn test_property_testing_integration() {
    ...
    context
        .property_tester
        .add_property("data_integrity", |data: &Vec<u8>| {
    ...
    info!(
        "Property tests completed: {} passed, {} failed",
    ...
    match result {
        Ok(received_data) => {
            info!(
                "Network transmission successful: {} bytes",
                received_data.len()
            );
        }
        Err(e) => {
            info!(
                "Network transmission failed (expected with packet loss): {:?}",
                e
            );
        }
    }
...
async fn test_cross_layer_metrics() {
    ...
    context
        .metrics_collector
        .record_crypto_operation("hpke_derive", Duration::from_micros(100))
        .await;
    context
        .metrics_collector
        .record_stream_bytes_sent(1024)
        .await;
    context
        .metrics_collector
        .record_network_latency(Duration::from_millis(50))
        .await;
...
async fn test_configuration_propagation() {
    ...
    // Verify all layers received the configuration update
    {
        ...
        assert!(layer_manager
            .get_layer_config("crypto")
            .contains("key_rotation_interval"));
        assert!(layer_manager
            .get_layer_config("stream")
...
async fn test_complete_data_pipeline() {
    ...
    context
        .event_bus
        .publish("data.mix", format!("Processing {} bytes", test_data.len()))
        .await;

    // 4. Data gets FEC encoded
    context
        .event_bus
        .publish("data.fec", "Applying forward error correction".into())
        .await;

    // 5. Data gets transmitted through transport layer
    context
        .event_bus
        .publish("data.transport", "Transmitting over network".into())
        .await;
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            let (private_key, public_key) =
                deriver.derive_keypair().expect("Failed to derive keypair");
