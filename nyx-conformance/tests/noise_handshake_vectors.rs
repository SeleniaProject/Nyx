#[cfg(feature = "kyber")]
use nyx_crypto::noise::kyber::{initiator_encapsulate, responder_decapsulate, responder_keypair};
...
fn noise_handshake_multiple_rounds() {
    for round in 0..10 {
        ...
        assert_eq!(
            key_init.0, key_resp.0,
            "Session keys must match in round {}",
            round
        );
    }
}
...
fn noise_handshake_corrupted_keys() {
    ...
    assert_ne!(
        key_init.0, key_resp.0,
        "Corrupted handshake should produce different keys"
    );
}
...
fn noise_handshake_performance() {
    ...
    assert!(
        avg_ms < 10.0,
        "Average handshake time {} ms is too slow",
        avg_ms
    );
}
