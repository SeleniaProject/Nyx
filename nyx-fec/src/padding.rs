...
mod tests {
    ...
    #[test]
    fn try_pack_exact_capacity_minus_header() -> Result<(), Box<dyn std::error::Error>> {
        let buf = vec![9u8; SHARD_SIZE - 2];
        let shard = try_pack_into_shard(&buf).ok_or("failed to pack")?;
    ...
    #[test]
    fn try_unpack_matches_unpack_for_valid_input() -> Result<(), Box<dyn std::error::Error>> {
        let data = b"abc";
        let shard = pack_into_shard(data);
        let a = unpack_from_shard(&shard);
        let b = try_unpack_from_shard(&shard).ok_or("failed to unpack")?;
        assert_eq!(a, b);
        Ok(())
    }
}
