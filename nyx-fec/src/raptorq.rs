...
impl AdaptiveRedundancyTuner {
    ...
    fn calculate_adaptive_redundancy(&self, current: &NetworkMetrics) -> Redundancy {
        ...
        // Apply all modifiers
        let final_tx = (base_tx * quality_modifier * bandwidth_modifier * stability_modifier).clamp(0.01, 0.9);
        let final_rx = (base_rx * quality_modifier * bandwidth_modifier * stability_modifier).clamp(0.01, 0.9);
    ...
    fn calculate_integral_error(&self) -> f32 {
        ...
        self.loss_window.iter()
            ...
            .sum::<f32>() / self.loss_window.len().max(1) as f32
    }
    ...
    pub fn get_statistics(&self) -> TunerStatistics {
        TunerStatistics {
            ...
            average_loss_rate: self.loss_window.iter().sum::<f32>() / self.loss_window.len().max(1) as f32,
