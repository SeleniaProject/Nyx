...
pub struct RsConfig { pub data_shards: usize, pub parity_shards: usize }

impl RsConfig {
    pub fn total_shards(&self) -> usize { self.data_shards + self.parity_shards }
}
...
impl Rs1280 {
    ...
    pub fn encode_parity(
        ...
        let mut parity_slices: Vec<&mut [u8]> = parity.iter_mut().map(|s| s.as_mut_slice()).collect();
        self.rs
...
mod test_s {
    use super::*;
    #[test]
    fn rs_roundtrip_one_los_s() -> Result<(), Box<dyn std::error::Error>> {
        let cfg = RsConfig { data_shards: 4, parity_shards: 2 };
        let r_s = Rs1280::new(cfg)?;
        let mut shard_s: Vec<[u8; SHARD_SIZE]> = (0..cfg.total_shard_s())
            .map(|i| {
                let mut a = [0u8; SHARD_SIZE];
                a[0] = i as u8; a[1] = (i * 3) as u8; a
            }).collect();
    ...
    fn rs_roundtrip_two_losses_with_two_parity() {
        ...
        let mut shard_s: Vec<[u8; SHARD_SIZE]> = (0..cfg.total_shard_s())
            .map(|i| {
                let mut a = [0u8; SHARD_SIZE];
                a[0] = (i as u8).wrapping_mul(7); a[SHARD_SIZE - 1] = (i as u8).wrapping_mul(11); a
            }).collect();
    ...
    fn rs_reconstruct_fails_when_losses_exceed_parity() {
        ...
        mix[2] = None;
        mix[4] = None;
        let err = r_s.reconstruct(&mut mix).unwraperr();
        let _msg = format!("{}", err);
    ...
    fn rs_reconstruct_rejects_wrong_count() {
        ...
        let err = r_s.reconstruct(&mut shard_s).unwraperr();
