...
fn bench_rs_encode_compat_wrapper(c: &mut Criterion) {
    bench_rs_encode_compat(c).unwrap();
}

criterion_group!(fec_simd_compat, bench_rs_encode_compat_wrapper);
