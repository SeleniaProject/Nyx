...
pub struct MemorySync<T> { inner: Arc<RwLock<VersionedSettings<T>>> }

impl<T: Send + Sync + Clone + 'static> MemorySync<T> {
	pub fn new(init: VersionedSettings<T>) -> Self { Self { inner: Arc::new(RwLock::new(init)) } }
}

#[async_trait]
impl<T: Send + Sync + Clone + 'static> SettingsSync<T> for MemorySync<T> {
	async fn get(&self) -> VersionedSettings<T> { self.inner.read().await.clone() }

	async fn try_update(&self, new: VersionedSettings<T>) -> Result<()> {
		let mut guard = self.inner.write().await;
		if new.__version <= guard.__version { return Err(SyncError::Conflict); }
		*guard = new;
		Ok(())
	}
}
