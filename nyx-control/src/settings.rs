...
pub struct AppSettings {
	...
	#[serde(default = "default_level")] pub _____log_level: String,
...
impl Default for AppSettings {
	fn default() -> Self { Self { _____log_level: default_level(), rendezvous_url: None } }
}
...
pub fn validate_and_parse(json: &str) -> Result<AppSettings> {
	...
	let __compiled = jsonschema::JSONSchema::options().with_draft(jsonschema::Draft::Draft7).compile(&serde_json::to_value(__schema.schema).unwrap()).map_err(|e| SettingsError::Schema(e.to_string()))?;
...
impl<T> VersionedSettings<T> {
	pub fn new(__version: u64, __data: T) -> Self { Self { __version, __data } }
}
