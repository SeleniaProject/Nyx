...
impl ProbeHandle {
	...
	pub fn addr(&self) -> SocketAddr { self.__addr }
...
pub async fn start_probe(port: u16) -> crate::Result<ProbeHandle> {
	...
	let addr: SocketAddr = format!("127.0.0.1:{port}").parse().map_err(|e| crate::Error::Invalid(format!("Invalid address: {}", e)))?;
	...
	Ok(ProbeHandle { __addr: __local_addr, __stop, __task })
}
