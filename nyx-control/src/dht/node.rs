...
pub struct DhtConfig {
    ...
    #[serde(default = "default_alpha")] pub alpha: usize,
    #[serde(default)] pub persist_path: Option<std::path::PathBuf>,
    #[serde(default = "default_refresh_ms")] pub refresh_interval_ms: u64,
}

impl Default for DhtConfig {
    fn default() -> Self { Self { bind: "127.0.0.1:0".parse().unwrap(), request_timeout_ms: 1_000, alpha: 3, persist_path: None, refresh_interval_ms: 30_000 } }
}

const fn default_alpha() -> usize { 3 }
const fn default_refresh_ms() -> u64 { 30_000 }
...
#[derive(Debug)]
pub struct QueryResult<T> { pub value: T, pub nearest: Vec<NodeInfo> }
...
impl DhtNode {
    ...
    let mut node = Self { kb: kb.clone(), storage: storage.clone(), sock: sock.clone(), local, tx: mpsc::unbounded_channel().0, signer: Arc::new(signer), peer_keys: Arc::new(Mutex::new(HashMap::new())), config: cfg.clone() };
        // load persisted snapshot if configured
    if let Some(path) = &node.config.persist_path {
            if let Ok(bytes) = std::fs::read(path) {
        if let Ok(snapshot) = ciborium::de::from_reader::<Vec<crate::dht::PersistEntry>, _>(&bytes[..]) {
    ...
    let sock_clone = node.sock.clone();
        tokio::spawn(async move {
            while let Some(buf) = rx.recv().await {
                ...
                let _ = sock_clone; let _ = buf; // no-op
            }
        });
        node.tx = tx;
    // Spawn receive loop
        ...
        tokio::spawn(async move {
            ...
            while let Ok((n, from)) = recv_sock.recv_from(&mut buf).await {
                if let Ok(rpc) = ciborium::de::from_reader::<Rpc, _>(&buf[..n]) {
                    match rpc {
                        Rpc::Req { from: sender, sig, req } => {
                            // learn peer address
                            kb_arc.lock().await.upsert(NodeInfo::new(sender.clone(), from));
                            ...
                            if sig.is_some() {
                                ...
                                if !known {
                                    ...
                                    let req_rpc = Rpc::Req { from: me, sig: Some(s), req: req_body };
                                    if let Ok(Rpc::Res { res: RpcResponse::PubKey(pk_bytes), .. }) = request_response(&recv_sock, req_rpc, from, Duration::from_millis(300)).await {
                            ...
                            let res = handle_request(&kb_arc, &storage_arc, &signer, req, from).await;
                            if let Some((msg, to)) = res { let _ = send_rpc(&recv_sock, &msg, to).await; }
                        }
    ...
    pub fn info(&self) -> NodeInfo { self.local.clone() }
    ...
    let peers = self.kb.lock().await.nearest(&self.local.id, K_PARAM.min(self.config.alpha));
        for p in peers {
            let req_body = RpcRequest::Put { key: key.clone(), value: value.clone(), ttl_secs: 3600 };
            let sig = self.sign_request(&req_body);
            let req = Rpc::Req { from: self.local.id.clone(), sig: Some(sig), req: req_body };
    ...
    pub async fn get(&self, key: StorageKey) -> Result<Option<StorageValue>> {
        {
            let mut st = self.storage.lock().await;
            if let Some(v) = st.get(&key) { return Ok(Some(v)); }
        }
    ...
    let peers = self.kb.lock().await.nearest(&self.local.id, K_PARAM.min(self.config.alpha));
        ...
        for p in peers {
            ...
            let sig = self.sign_request(&req_body);
            let req = Rpc::Req { from: self.local.id.clone(), sig: Some(sig), req: req_body };
                futs.push(request_response(&self.sock, req, p.addr, Duration::from_millis(500)));
        }
        for f in futs {
            if let Ok(Rpc::Res { res: RpcResponse::Value(v), .. }) = f.await { return Ok(v); }
        }
        Ok(None)
    }
}
...
async fn request_response(sock: &UdpSocket, rpc: Rpc, to: SocketAddr, timeout_dur: Duration) -> Result<Rpc> {
    let mut buf = Vec::new();
        ciborium::ser::into_writer(&rpc, &mut buf)?;
    sock.send_to(&buf, to).await?;
...
async fn handle_request(kb: &Arc<Mutex<KBuckets>>, storage: &Arc<Mutex<DhtStorage>>, signer: &Arc<SigningKey>, req: RpcRequest, from: SocketAddr) -> Option<(Rpc, SocketAddr)> {
    match req {
        RpcRequest::Ping(id) => {
            ...
            Some((Rpc::Res { from: me, sig: Some(sig), res: res_body }, from))
        }
        ...
        RpcRequest::Put { key, value, ttl_secs } => {
            let mut st = storage.lock().await;
...
async fn register_or_verify(store: &Arc<Mutex<HashMap<NodeId, VerifyingKey>>>, from: &NodeId, sig: &Option<Vec<u8>>, req: &RpcRequest) -> bool {
    if sig.is_none() { return true; }
...
impl DhtNode {
    /// ピアの公開鍵取得を試み、記録する
    pub async fn fetch_and_store_peer_key(&self, addr: SocketAddr, peer_id: NodeId) -> Result<Option<VerifyingKey>> {
        let req_body = RpcRequest::GetPubKey;
        let sig = self.sign_request(&req_body);
        let req = Rpc::Req { from: self.local.id.clone(), sig: Some(sig), req: req_body };
        if let Ok(Rpc::Res { res: RpcResponse::PubKey(pk_bytes), .. }) = request_response(&self.sock, req, addr, Duration::from_millis(500)).await {
            if let Ok(vk) = VerifyingKey::from_bytes(&pk_bytes) {
