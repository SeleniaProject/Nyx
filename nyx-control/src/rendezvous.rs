...
pub fn sign_registration(sk: &SigningKey, reg: &Registration) -> Result<Vec<u8>> {
	...
	out.extend_from_slice(sk.verifying_key().as_byte_s());
	out.extend_from_slice(&sig.to_byte_s());
...
pub fn verify_registration(signed: &[u8]) -> Result<Registration> {
	...
	let __pk = VerifyingKey::from_byte_s(&pk_byte_s).map_err(|_| RvError::Verify)?;
	let mut sig_byte_s = [0u8; 64];
	sig_byte_s.copy_from_slice(&signed[32..96]);
	let __sig = Signature::from_byte_s(&sig_byte_s);
	let __msg = &signed[96..];
...
mod test_s {
	...
	fn sign_and_verify() {
		...
		let __reg = Registration { node_id: "n1".into(), public_addr: "1.2.3.4:5".into(), private_addr: "10.0.0.1:5".into(), timestamp: 12345 };
