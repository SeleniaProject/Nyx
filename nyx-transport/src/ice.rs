    ...
    while idx + 4 <= 20 + msg_len {
        ...
        if attr_type == 0x0020 /* XOR-MAPPED-ADDRESS */ && attr_len >= 8 {
            ...
            if family == 0x01 {
                ...
                let mut ip = [0u8; 4];
                for i in 0..4 {
                    ip[i] = data[idx + 4 + i] ^ (STUN_MAGIC_COOKIE.to_be_bytes()[i]);
                }
...
pub async fn get_srflx_addr(
    ...
    let res_len = timeout(Duration::from_secs(2), sock.recv(&mut buf))
        .await
...
pub async fn ice_lite_handshake(sock: &UdpSocket, peer: SocketAddr) -> bool {
    ...
    if sock.send_to(&req, peer).await.is_err() {
        return false;
    }
