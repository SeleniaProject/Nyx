...
impl Default for TcpFallbackConfig {
    fn default() -> Self {
        // Allow environment overrides for defaults (seconds). This keeps behavior configurable
        // for constrained devices without changing code.
        let keepalive_s = std::env::var("NYX_TCP_KEEPALIVE")
            .ok()
            .and_then(|v| v.parse::<u64>().ok())
            .unwrap_or(30);
        let idle_s = std::env::var("NYX_TCP_IDLE")
            .ok()
            .and_then(|v| v.parse::<u64>().ok())
            .unwrap_or(300);
        Self {
            ...
            keepalive_interval: Duration::from_secs(keepalive_s),
            max_idle_time: Duration::from_secs(idle_s),
            buffer_size: 8192,
...
impl TcpFallbackConfig {
    /// Adjust timers for low power preference: extend keepalive and idle timers for battery savings.
    pub fn apply_low_power(&mut self) {
        // Increase keepalive to 60s and idle timeout to 600s in low power mode (override by env if provided)
        let keepalive_lp = std::env::var("NYX_TCP_KEEPALIVE_LP")
            .ok()
            .and_then(|v| v.parse::<u64>().ok())
            .unwrap_or(60);
        let idle_lp = std::env::var("NYX_TCP_IDLE_LP")
            .ok()
            .and_then(|v| v.parse::<u64>().ok())
            .unwrap_or(600);
        self.keepalive_interval = Duration::from_secs(keepalive_lp);
        self.max_idle_time = Duration::from_secs(idle_lp);
    }
}
...
impl TcpEncapListener {
    ...
    pub async fn bind_with_config(port: u16, config: TcpFallbackConfig) -> std::io::Result<Self> {
        let public = std::env::var("NYX_TCP_PUBLIC")
            .ok()
                ...
                loop {
                    match listener.accept().await {
                        Ok((mut stream, addr)) => {
                            ...
                            if connections_for_accept.read().await.len()
                                >= config_for_accept.max_connections
                            {
                            ...
                            tokio::spawn(
                                async move {
                                        ...
                                        match read_result {
                                            Ok(Ok(Some(packet))) => {
                                                ...
                                                if tx_clone.send((addr, packet)).await.is_err() {
                                                    debug!(
                                                        "Channel closed, terminating connection {}",
                                                        addr
                                                    );
                                                    break;
                                                }
                                    ...
                                    if let Some(stats) =
                                        connections_for_task.read().await.get(&addr)
                                    {
        ...
        for attempt in 0..config.retry_attempts {
            match Self::connect_with_config(addr, config.clone()).await {
                Ok(conn) => {
                    if attempt > 0 {
                        info!(
                            "tcp_fallback: connected to {} after {} retries",
                            addr, attempt
                        );
                    }
                    return Ok(conn);
                ...
                Err(e) => {
                    ...
                    if attempt < config.retry_attempts - 1 {
                        ...
                        warn!(
                            "tcp_fallback: connection attempt {} failed, retrying in {:?}",
                            attempt + 1,
                            backoff
                        );
        ...
        for attempt in 0..self.config.retry_attempts {
            match self.send(data).await {
                ...
                Err(e) if attempt < self.config.retry_attempts - 1 => {
                    warn!(
                        "tcp_fallback: send attempt {} failed: {}, retrying",
                        attempt + 1,
                        e
                    );
...
mod tests {
    ...
    async fn test_connection_with_config() {
        ...
        let mut listener = TcpEncapListener::bind_with_config(4481, config.clone())
            .await
            .unwrap();
        let conn = TcpEncapConnection::connect_with_config("127.0.0.1:4481", config)
            .await
            .unwrap();
    ...
    async fn test_listener_connection_limits() {
        ...
        let listener = TcpEncapListener::bind_with_config(4483, config)
            .await
            .unwrap();
