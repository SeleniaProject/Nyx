...
use nyx_transport::{UdpEndpoint, tcp_fallback};
    ...
    for size in message_sizes {
        group.bench_with_input(
            ...
            |b, &size| {
                ...
                b.iter(|| {
                    // Send message
                    endpoint_a.send_to_buffered(black_box(&message), addr_b).unwrap();
...
fn tcp_connection_pool_benchmark(c: &mut Criterion) {
    ...
    group.bench_function("simple_connect", |b| {
        b.iter(|| {
            let _result = tcp_fallback::try_connect(
                black_box(server_addr),
