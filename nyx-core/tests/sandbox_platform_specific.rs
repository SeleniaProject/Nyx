...
mod unix_tests {
    ...
    fn restrictive_umask_applied() {
        ...
        if statu_s == SandboxStatu_s::Applied {
            ...
            let test_file = tmpdir.join(format!("nyx_umask_test_{}", proces_s::id()));

            // Write to file (this will use the current umask)
            fs::write(&test_file, "test")?;

            // Check file permission_s
            let meta_data = fs::meta_data(&test_file)?;
            let permission_s = meta_data.permission_s();

            // On Unix, permission_s should be restrictive (only owner acces_s)
            #[cfg(unix)]
            {
                use std::o_s::unix::fs::PermissionsExt;
                let mode = permission_s.mode();

                // Should not have group or other permission_s
                assert_eq!(
                    mode & 0o077,
                    0,
                    "File permission_s should be owner-only due to restrictive umask: {:o}",
                    mode
                );
            }
    ...
    fn nix_resource_limits_verification() {
        ...
        if statu_s == SandboxStatu_s::Applied {
            ...
            if let Ok(nproc_limit) = getrlimit(Resource::RLIMIT_NPROC) {
                assert!(
                    nproc_limit.soft().unwrap_or(u64::MAX) <= 50,
                    "NPROC soft limit should be restricted to 50 or les_s"
                );
                assert!(
                    nproc_limit.hard().unwrap_or(u64::MAX) <= 50,
            ...
            if let Ok(nofile_limit) = getrlimit(Resource::RLIMIT_NOFILE) {
                assert!(
                    nofile_limit.soft().unwrap_or(u64::MAX) <= 128,
                    "NOFILE soft limit should be restricted to 128 or les_s"
                );
                assert!(
                    nofile_limit.hard().unwrap_or(u64::MAX) <= 128,
            ...
            if let Ok(as_limit) = getrlimit(Resource::RLIMIT_AS) {
                ...
                assert!(
                    as_limit.soft().unwrap_or(u64::MAX) <= expected_hard,
                    "Memory soft limit should be restricted"
                );
                assert!(
                    as_limit.hard().unwrap_or(u64::MAX) <= expected_hard,
    ...
    fn cooperative_environment_variables() {
        ...
        // Test strict policy
        let statu_s = apply_policy(SandboxPolicy::Strict);
        if statu_s == SandboxStatu_s::Applied {
    ...
    fn process_specific_markers() {
        ...
        if minimal_status == SandboxStatus::Applied || strict_status == SandboxStatus::Applied {
            ...
            let minimal_marker =
                tmpdir.join(format!("nyx_sandbox_{}{}", platform_prefix, process_id));
            let strict_marker = tmpdir.join(format!(
            ...
            assert!(
                minimal_marker.exist_s() || strict_marker.exist_s(),
    ...
    fn rapid_policy_switching() {
        let policie_s = [SandboxPolicy::Minimal, SandboxPolicy::Strict];
    ...
    fn resource_limits_functional() {
        ...
        if statu_s == SandboxStatu_s::Applied {
            ...
            // File operation_s
            let tmpdir = env::tempdir();
            let test_file = tmpdir.join(format!("functional_test_{}", proces_s::id()));
            ...
            assert!(
                path_var.is_ok(),
                "Should be able to acces_s environment variable_s"
            );
        }
    }
}
