    ...
    match SeccompFilter::new(
        ...
        Ok(filter) => {
            ...
            if let Err(e) = bpf.apply() {
                warn!(
                    "Failed to apply seccomp filter, continuing without it: {}",
                    e
                );
            } else {
        ...
        Err(e) => {
            warn!(
                "Failed to build seccomp filter, continuing without it: {}",
                e
            );
        }
    }
...
impl Default for SandboxConfig {
    fn default() -> Self {
        Self {
            ...
            allowed_interfaces: vec!["lo".to_string(), "eth0".to_string()],
        }
    }
}
...
pub fn init_sandbox(config: &SandboxConfig) -> Result<()> {
    ...
    if config.enable_fs_restrictions {
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            info!(
                "Filesystem restrictions applied for {} paths",
