...
impl I18n {
	...
	pub fn insert_resource(&mut self, lang: &str, ftl: &str) -> anyhow::Result<()> {
		let resource = FluentResource::try_new(ftl.to_string()).map_err(|(_, e)| anyhow::anyhow!("fluent parse error: {e:?}"))?;
		...
		bundle.add_resource(resource).map_err(|e| anyhow::anyhow!("bundle add resource: {e:?}"))?;
	...
	pub fn format<'a>(&self, key: &str, args: Option<&FluentArgs<'a>>) -> String {
		let Some(bundle) = self.bundles.get("en") else { return key.to_string() };
		...
		if errors.is_empty() { result } else { key.to_string() }
	}
