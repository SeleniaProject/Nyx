...
impl PluginFramework {
    ...
    pub async fn handle_plugin_frame(
        ...
        if frame_type < 0x50 || frame_type > 0x5F {
            return Err(PluginError::InvalidHeader(format!(
        ...
        debug!(
            "Received plugin frame: type=0x{:02X}, plugin_id={}, data_len={}",
            frame_type,
            header.id,
            header.data.len()
        );
    ...
    async fn handle_plugin_message(
        ...
        match message {
            ...
            PluginMessage::Control(control) => {
                debug!(
                    "Processing control message for plugin {}: {:?}",
                    plugin.id, control
                );
                Ok(())
            }
    ...
    pub async fn create_handshake_frame(&self, plugin_id: u32) -> Result<Vec<u8>, PluginError> {
        let plugins = self.plugins.read().await;
        let plugin = plugins
            .get(&plugin_id)
...
mod tests {
    ...
    fn test_cbor_serialization() {
        ...
        let deserialized: PluginHeader =
            cbor_serde::from_slice(&serialized).expect("deserialize header");
