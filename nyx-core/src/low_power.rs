...
pub fn screen_off_ratio(events: &[(TimestampMs, ScreenState)]) -> f64 {
    ...
    let Some(&(TimestampMs(start), _)) = events.first() else {
        return 0.0;
    };
    let Some(&(TimestampMs(end), _)) = events.last() else {
        return 0.0;
    };
    if end <= start {
        return 0.0;
...
impl InactivityTrigger {
    pub fn new(threshold: Duration, rate_per_sec: f64, now: TimestampMs) -> Self {
        Self {
            threshold,
            limiter: RateLimiter::new(1.0, rate_per_sec),
            last_activity: now,
        }
    }
    pub fn record_activity(&mut self, ts: TimestampMs) {
