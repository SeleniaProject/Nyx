...
#[cfg(feature = "telemetry")]
impl LowPowerTelemetry {
    fn new() -> Self {
        Self {
            cover_packets_metric: Mutex::new(BasicMetrics::new()),
            push_notifications_metric: Mutex::new(BasicMetrics::new()),
            suppressed_cover_packets_metric: Mutex::new(BasicMetrics::new()),
        }
    }
}
...
impl LowPowerManager {
    ...
    #[cfg(feature = "mobile_ffi")]
    pub fn with_mobile_ffi(
        push_service: Option<Arc<dyn PushNotificationService>>,
    ) -> Result<Self, LowPowerError> {
        ...
        tokio::spawn(async move {
            ...
            while let Some(screen_on) = screen_rx.recv().await {
                let new_state = if screen_on {
                    ...
                    match (
                        screen_detector.is_power_save_mode(),
                        screen_detector.get_battery_level(),
                    ) {
                        (Ok(true), _) => PowerState::PowerSaveMode,
                        (_, Ok(level)) if level < BATTERY_CRITICAL_THRESHOLD => {
                    ...
                    *stats_guard
                    ...
                    if matches!(new_state, PowerState::ScreenOn) {
                        if let Some(pg) = push_gateway_opt.read().unwrap().as_ref().cloned() {
                            tokio::spawn(async move {
                                let _ = pg.resume_low_power_session().await;
                            });
                        }
                    }
                }
    ...
    pub fn queue_message(&self, message: DelayedMessage) -> Result<(), LowPowerError> {
        ...
        debug!(
            "Message queued for delayed sending, queue size: {}",
            queue.len()
        );
        Ok(())
    }
    ...
    pub async fn send_push_notification(
        ...
        let push_service =
            self.push_service
                .as_ref()
                .ok_or(LowPowerError::PushNotificationError(
                    "Push service not configured".to_string(),
                ))?;

        let device_token = self.device_token.read().unwrap().clone().ok_or(
            LowPowerError::PushNotificationError("Device token not set".to_string()),
        )?;
        ...
        push_service
            .send_notification(&device_token, &push_message)
            .await?;
        ...
        #[cfg(feature = "telemetry")]
        {
            if let Ok(mut m) = self.telemetry.push_notifications_metric.lock() {
                m.increment();
            }
        }
            ...
            loop {
                ...
                match screen_detector.get_battery_level() {
                    Ok(level) => {
                        ...
                        stats_guard
                            .battery_history
                            .retain(|(ts, _)| timestamp - ts < 86400);
            ...
            loop {
                ...
                if should_generate {
                    ...
                    let packet_size = pattern.size_range.0
                        + (rand::random::<usize>()
                            % (pattern.size_range.1 - pattern.size_range.0).max(1));
                    ...
                    let intensity_snapshot;
                    {
                        ...
                        if intensity_snapshot < 1.0 && intensity_snapshot > 0.0 {
                            ...
                            // scale: (1/intensity) - 1
                            let suppressed_estimate =
                                ((1.0 / intensity_snapshot) - 1.0).round() as u64;
                            stats_guard.suppressed_cover_packets += suppressed_estimate;
                            #[cfg(feature = "telemetry")]
                            if let Ok(mut m) = telemetry.suppressed_cover_packets_metric.lock() {
                                for _ in 0..suppressed_estimate {
                                    m.increment();
                                }
                            }
                        }
                    ...
                    #[cfg(feature = "telemetry")]
                    if let Ok(mut m) = telemetry.cover_packets_metric.lock() {
                        m.increment();
                    }
                    ...
                    let variance =
                        base_ms * pattern.interval_variance * (rand::random::<f64>() - 0.5) * 2.0;
                    let next_interval =
                        Duration::from_millis((base_ms + variance).max(100.0) as u64);
                    ...
                    PowerState::ScreenOn => {
                        ...
                        for message in messages_to_send {
                            ...
                            if let Some(tx) = &*message_packet_sink.lock().unwrap() {
                                if tx
                                    .send((message.destination.clone(), message.payload.clone()))
                                    .is_ok()
                                {
                                    sent_via_sink = true;
                                }
                        ...
                        if let Some(push_svc) = &push_service {
                            ...
                            for message in to_push {
                                ...
                                if let Some(token) = token {
                                    ...
                                    if let Err(e) =
                                        push_svc.send_notification(&token, &push_message).await
                                    {
...
mod tests {
    ...
    impl TestScreenDetector {
        ...
        fn set_battery(&self, lvl: u8) {
            self.battery_level.store(lvl, Ordering::Relaxed);
        }
    }
    ...
    async fn test_low_power_state_transitions_and_cover_generation() {
        ...
        manager
            .update_cover_pattern(CoverTrafficPattern {
        ...
        while attempts < 20 {
            ...
            if manager.get_stats().cover_packets_generated > 0 {
                have_packets = true;
                break;
            }
            attempts += 1;
        ...
        assert!(
            have_packets,
            "expected cover packets when screen off within timeout"
        );
    ...
    async fn test_auto_resume_on_screen_on_triggers_push_gateway() {
        ...
        assert!(
            success,
            "expected push gateway resume to be invoked after screen on transition"
        );
    }
}
...
impl LowPowerManager {
    ...
    async fn send_message_via_nyx(message: &DelayedMessage) -> Result<(), LowPowerError> {
        ...
        let endpoint = message.destination.parse::<NodeEndpoint>().map_err(|e| {
            LowPowerError::PushNotificationError(format!("Invalid destination: {}", e))
        })?;

        // Send via transport layer
        // This would integrate with nyx-transport layer
        trace!(
            "Sending {} bytes to {} via Nyx transport",
            message.payload.len(),
            endpoint
        );
