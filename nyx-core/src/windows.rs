...
// Note: This module uses a minimal unsafe block to obtain the current process handle
// via Win32 API. All other interactions remain within safe wrappers.
...
///
/// This uses a minimal unsafe call to fetch the current process handle; all other logic is safe.
pub fn apply_process_isolation(config: Option<WindowsIsolationConfig>) -> io::Result<()> {
    ...
    // Configure working set bounds as a practical memory constraint
