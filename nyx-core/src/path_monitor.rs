...
impl PathPerformanceMonitor {
    pub fn new(path_id: impl Into<String>) -> Self {
        Self {
            ...
            latency_samples: Arc::new(RwLock::new(VecDeque::with_capacity(
                PERFORMANCE_SAMPLE_WINDOW,
            ))),
            bandwidth_samples: Arc::new(RwLock::new(VecDeque::with_capacity(
                PERFORMANCE_SAMPLE_WINDOW,
            ))),
    ...
    pub async fn start_monitoring(&self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        if self.enabled.swap(true, std::sync::atomic::Ordering::SeqCst) {
            return Ok(());
        }
        ...
        let handle = tokio::spawn(async move {
            ...
            while enabled.load(std::sync::atomic::Ordering::SeqCst) {
                ...
                let point = super::path_monitor::PerformanceDataPoint {
                    timestamp: SystemTime::now(),
                    latency_ms: m.current_latency_ms,
                    bandwidth_mbps: m.current_bandwidth_mbps,
                    packet_loss_rate: m.packet_loss_rate,
                    reliability_score: m.reliability_score,
                };
                drop(m);
                drop(hist);
                let mut hw = history.write().await;
                hw.push_back(point);
                if hw.len() > PERFORMANCE_SAMPLE_WINDOW {
                    hw.pop_front();
                }
            }
        });
    ...
    pub async fn stop_monitoring(&self) {
        ...
        if let Some(h) = self.monitoring_task.lock().await.take() {
            h.abort();
        }
    }
    ...
    async fn compute_trend_and_maybe_alert(
        ...
        if recent.len() >= 2 {
            ...
            let volatility = if rel_vals.len() > 1 {
                ...
                (rel_vals.iter().map(|v| (v - mean).powi(2)).sum::<f64>() / rel_vals.len() as f64)
                    .sqrt()
            } else {
                0.0
            };
            let overall = reliability_delta - latency_delta / 100.0;
            m.performance_trend = if volatility > 0.3 {
        ...
        if m.reliability_score < PERFORMANCE_ALERT_THRESHOLD {
            if let Some(cb) = alert_cb.lock().await.as_ref() {
                cb(&m);
            }
        }
    }
    ...
    async fn push_point_and_update(&self) {
        let snapshot = { self.metrics.read().await.clone() };
        {
            ...
            hw.push_back(PerformanceDataPoint {
                timestamp: SystemTime::now(),
                latency_ms: snapshot.current_latency_ms,
                bandwidth_mbps: snapshot.current_bandwidth_mbps,
    ...
    pub async fn record_latency(&self, latency_ms: f64) {
        let mut s = self.latency_samples.write().await;
        s.push_back(latency_ms);
        if s.len() > PERFORMANCE_SAMPLE_WINDOW {
            s.pop_front();
        }
        let avg = s.iter().sum::<f64>() / s.len() as f64;
        {
            let mut m = self.metrics.write().await;
            m.current_latency_ms = latency_ms;
            m.avg_latency_ms = avg;
    ...
    pub async fn record_bandwidth(&self, bandwidth_mbps: f64) {
        let mut s = self.bandwidth_samples.write().await;
        s.push_back(bandwidth_mbps);
        if s.len() > PERFORMANCE_SAMPLE_WINDOW {
            s.pop_front();
        }
        let avg = s.iter().sum::<f64>() / s.len() as f64;
        {
            let mut m = self.metrics.write().await;
            m.current_bandwidth_mbps = bandwidth_mbps;
            m.avg_bandwidth_mbps = avg;
            m.last_updated = SystemTime::now();
        }
    ...
    pub async fn record_transmission(&self, bytes_tx: u64, bytes_rx: u64, success: bool) {
        {
            let mut m = self.metrics.write().await;
            m.bytes_transmitted += bytes_tx;
            m.bytes_received += bytes_rx;
            if success {
                m.successful_transmissions += 1;
            } else {
                m.failed_transmissions += 1;
            }
            let total = m.successful_transmissions + m.failed_transmissions;
            if total > 0 {
                m.reliability_score = m.successful_transmissions as f64 / total as f64;
                m.packet_loss_rate = 1.0 - m.reliability_score;
                m.throughput_efficiency = m.reliability_score;
            }
            m.last_updated = SystemTime::now();
        }
        self.push_point_and_update().await;
    }
    pub async fn set_alert_callback<F: Fn(&PathPerformanceMetrics) + Send + Sync + 'static>(
        &self,
        cb: F,
    ) {
        *self.alert_callback.lock().await = Some(Box::new(cb));
    }
...
pub struct PathPerformanceRegistry {
    monitors: RwLock<HashMap<String, Arc<PathPerformanceMonitor>>>,
    started_at: Instant,
}
impl PathPerformanceRegistry {
    pub fn new() -> Self {
        Self {
    ...
    pub async fn get_or_create(&self, id: &str) -> Arc<PathPerformanceMonitor> {
        if let Some(m) = self.monitors.read().await.get(id) {
            return m.clone();
        }
        let mut w = self.monitors.write().await;
        let e = w
            .entry(id.to_string())
            .or_insert_with(|| Arc::new(PathPerformanceMonitor::new(id.to_string())));
        e.clone()
    }
    pub async fn global_stats(&self) -> GlobalPathStats {
        ...
        for (id, m) in map.iter() {
            let met = m.get_metrics().await;
            let score = (met.reliability_score + (1.0 / (1.0 + met.avg_latency_ms / 100.0))) / 2.0;
            sum_score += score;
            sum_loss += met.packet_loss_rate;
            total += 1;
            success += met.successful_transmissions;
            fail += met.failed_transmissions;
            match best {
                Some((_, b)) if score <= b => {}
            ...
            match worst {
                Some((_, w)) if score >= w => {}
                _ => worst = Some((id.clone(), score)),
            }
        }
        GlobalPathStats {
            active_paths: total as u32,
            avg_performance_score: if total > 0 {
                sum_score / total as f64
            } else {
                1.0
            },
            global_packet_loss_rate: if total > 0 {
                sum_loss / total as f64
            } else {
                0.0
            },
            total_successful_transmissions: success,
            total_failed_transmissions: fail,
            monitoring_uptime_secs: self.started_at.elapsed().as_secs(),
...
mod tests {
    ...
    #[tokio::test]
    async fn basic_flow() {
        let m = PathPerformanceMonitor::new("p1");
        m.start_monitoring().await.unwrap();
        m.record_latency(10.0).await;
        m.record_bandwidth(50.0).await;
        m.record_transmission(100, 100, true).await;
        let met = m.get_metrics().await;
        assert_eq!(met.current_latency_ms, 10.0);
        assert_eq!(met.current_bandwidth_mbps, 50.0);
        m.stop_monitoring().await;
    }
}
