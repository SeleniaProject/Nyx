...
impl LatencyWindow {
	pub fn new(cap: usize) -> Self { Self { samples: std::collections::VecDeque::with_capacity(cap), cap } }
	pub fn push(&mut self, v_ms: u128) {
		if self.samples.len() == self.cap { self.samples.pop_front(); }
	...
	pub fn is_degraded(&self, baseline_ms: u128, factor: f64) -> bool {
		match self.avg() { Some(avg) => avg >= baseline_ms as f64 * factor, None => false }
	}
}
...
mod tests {
	...
	fn window_avg_and_degraded() {
		...
		w.push(200);
		// now holds 110,120,200 => avg ~143.33
