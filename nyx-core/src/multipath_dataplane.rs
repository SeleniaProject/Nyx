...
impl MultipathDataPlane {
    ...
    pub fn add_path(&self, path_id: u8, initial_rtt: Duration) -> Result<(), MultipathError> {
        ...
        info!(
            "Added path {} with RTT {:?} and weight {:.3}",
            path_id.as_u8(),
            initial_rtt,
            path_info.weight
        );
    ...
    pub fn send_packet(
        &self,
        data: Vec<u8>,
    ) -> Result<(PathId, MultipathPacketHeader, Vec<u8>), MultipathError> {
        ...
        debug!(
            "Sending packet on path {} with {} hops",
            path_id.as_u8(),
            hop_count
        );
    ...
    pub fn receive_packet(
        ...
        debug!(
            "Received packet on path {}, extracted {} ordered packets",
            header.path_id,
            ordered_packets.len()
        );
    ...
    pub fn update_path_statistics(
        ...
        let now_ns = SystemTime::now()
            ...
            .unwrap_or_default()
            .as_nanos() as u64;
        let rtt = if now_ns >= header.timestamp {
            Duration::from_nanos(now_ns - header.timestamp)
        } else {
        ...
        // Update path info
        {
            ...
            if let Some(path_info) = paths.get_mut(&path_id.as_u8()) {
                ...
                path_info.jitter = Duration::from_nanos(
                    (path_info.jitter.as_nanos() as f64 * 0.8 + rtt_diff.as_nanos() as f64 * 0.2)
                        as u64,
                );
        ...
        if hop_config.should_adapt() {
            ...
            for (path_id, path_info) in paths.iter() {
                ...
                if new_hops != current_hops {
                    ...
                    info!(
                        "Adapted path {} hop count: {} -> {}",
                        path_id, current_hops, new_hops
                    );
                }
            }
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                for (path_id, buffer) in buffers.iter_mut() {
                    ...
                    if !expired.is_empty() {
                        warn!(
                            "Path {}: {} packets expired from reorder buffer",
                            path_id,
                            expired.len()
                        );
                    }
    ...
    fn update_quality_scores(&mut self) -> Result<(), MultipathError> {
        for (path_id, metrics) in self.path_metrics.iter_mut() {
            ...
            if metrics.quality_score < self.quality_threshold {
                warn!(
                    "Path {} quality below threshold: {:.3}",
                    path_id.as_u8(),
