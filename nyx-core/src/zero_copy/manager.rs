...
impl CriticalPath {
    ...
    pub async fn start_processing(
        &self,
        context_id: String,
    ) -> Result<ProcessingContext, ZeroCopyError> {
    ...
    pub async fn allocate_buffer(
        ...
        stage: Stage,
        size: usize,
    ) -> Result<ZeroCopyBuffer, ZeroCopyError> {
    ...
    pub async fn process_crypto_stage(
        &self,
        context_id: &str,
        input_data: &[u8],
    ) -> Result<ZeroCopyBuffer, ZeroCopyError> {
        ...
        let mut output_buffer = self
            .allocate_buffer(context_id, Stage::Crypto, output_size)
            .await?;
        ...
        debug!(
            "Processed {} bytes through crypto stage for context {}",
            input_data.len(),
            context_id
        );
        Ok(output_buffer)
    ...
    pub async fn process_fec_stage(
        &self,
        context_id: &str,
        input_buffer: &ZeroCopyBuffer,
    ) -> Result<Vec<ZeroCopyBuffer>, ZeroCopyError> {
        ...
        for symbol_idx in 0..total_symbols {
            let symbol_buffer = self
                .allocate_buffer(context_id, Stage::Fec, symbol_size)
                .await?;
        ...
        debug!(
            "Processed {} symbols ({} source + {} repair) through FEC stage for context {}",
    ...
    pub async fn process_transmission_stage(
        &self,
        context_id: &str,
        symbol_buffers: &[ZeroCopyBuffer],
    ) -> Result<Vec<ZeroCopyBuffer>, ZeroCopyError> {
        ...
        for (_idx, symbol_buffer) in symbol_buffers.iter().enumerate() {
            ...
            let mut tx_buffer = self
                .allocate_buffer(context_id, Stage::Transmission, total_size)
                .await?;
            ...
            if self.config.enable_zero_copy && symbol_buffer.can_reuse() {
                ...
                tx_buffer
                    .as_mut()
                    .extend(std::iter::repeat(0u8).take(header_size));
            } else {
        ...
        debug!(
            "Processed {} transmission buffers for context {}",
            transmission_buffers.len(),
            context_id
        );
    ...
    pub async fn process_packet(
        ...
        // Transmission stage
        let tx_outputs = self
            .process_transmission_stage(&context_id, &fec_outputs)
            .await?;
    ...
    pub async fn finish_processing(&self, context_id: &str) -> Result<(), ZeroCopyError> {
        ...
        if let Some(context) = contexts.remove(context_id) {
            ...
            debug!(
                "Finished processing context {} in {:?}",
                context_id, duration
            );
        }
...
impl ZeroCopyManager {
    ...
    pub async fn create_critical_path(
        &self,
        path_id: String,
    ) -> Result<Arc<CriticalPath>, ZeroCopyError> {
        ...
        let path = Arc::new(CriticalPath::new(
            path_id.clone(),
            self.config.default_path_config.clone(),
        ));
...
impl AggregatedMetrics {
    pub fn merge(&mut self, metrics: AllocationMetrics) {
        ...
        self.average_zero_copy_ratio = (self.average_zero_copy_ratio * current_paths
            + metrics.zero_copy_ratio)
            / (current_paths + 1.0);
        self.average_reduction_ratio = (self.average_reduction_ratio * current_paths
