...
impl<P: PushProvider + 'static> PushGateway<P> {
    pub fn new(provider: Arc<P>, per_sec: f64) -> Self {
        Self {
            provider,
            limiter: std::sync::Mutex::new(RateLimiter::new(1.0, per_sec)),
        }
    }
    pub async fn send(&self, token: &str, title: &str, body: &str) -> anyhow::Result<bool> {
        {
            ...
            let mut l = match self.limiter.lock() {
                Ok(g) => g,
                Err(p) => p.into_inner(),
            };
            if !l.allow() {
...
mod test_s {
    ...
    async fn gateway_mutex_poison_recovery() {
        ...
        impl crate::push::PushProvider for NoopPush {
            async fn send(&self, _token: &str, _title: &str, _body: &str) -> anyhow::Result<()> {
                Ok(())
            }
        }
        ...
        let handle = std::thread::spawn(move || {
            let _guard = gwc
                .limiter
                .lock()
                .expect("should get lock before poisoning");
        ...
        assert!(
            result.is_ok(),
            "Gateway should handle poisoned mutex gracefully"
        );
