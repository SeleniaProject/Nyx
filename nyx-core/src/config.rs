...
impl CoreConfig {
    ...
    pub fn load_from_file(path: impl AsRef<Path>) -> Result<Self> {
        ...
        let cfg: Self =
            toml::from_str(&_data).map_err(|e| Error::config(format!("_toml parse error: {e}")))?;
        cfg.validate()?;
        Ok(cfg)
    }
    ...
    pub fn from_env() -> Result<Self> {
        ...
        if let Ok(v) = std::env::var("NYX_LOG_LEVEL") {
            cfg.log_level = v;
        }
        if let Ok(v) = std::env::var("NYX_ENABLE_MULTIPATH") {
            cfg.enable_multipath = matches!(v.as_str(), "1" | "true" | "TRUE" | "True");
        }
        cfg.validate()?;
        Ok(cfg)
    }
...
impl CoreConfigBuilder {
    ...
    pub fn enable_multipath(mut self, enabled: bool) -> Self {
    ...
    pub fn build(self) -> Result<CoreConfig> {
        ...
        if let Some(v) = self.enable_multipath {
            cfg.enable_multipath = v;
        }
        cfg.validate()?;
        Ok(cfg)
    }
...
impl QuicConfig {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.keep_alive_interval_secs == 0 {
            return Err(Error::config(
                "keep_alive_interval_secs must be greater than 0".to_string(),
            ));
        }
        if self.keep_alive_interval_secs >= self.idle_timeout_secs {
            return Err(Error::config(
                "keep_alive_interval_secs must be less than idle_timeout_secs".to_string(),
            ));
        }
