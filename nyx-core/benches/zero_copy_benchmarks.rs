...
use nyx_core::zero_copy::{
    AllocationTracker, BufferPool, CriticalPath, CriticalPathConfig, Stage, ZeroCopyManager,
...
fn bench_complete_pipeline(c: &mut Criterion) {
    ...
    for packet_size in [1024, 4096, 8192, 16384].iter() {
        group.bench_with_input(
            BenchmarkId::new("process_packet", packet_size),
            packet_size,
            |b, &size| {
...
fn bench_concurrent_processing(c: &mut Criterion) {
    ...
    for concurrency in [1, 4, 8, 16].iter() {
        group.bench_with_input(
            BenchmarkId::new("concurrent_packets", concurrency),
            concurrency,
            |b, &conc| {
                let config = CriticalPathConfig::default();
                ...
                b.iter(|| {
                    rt.block_on(async {
                        ...
                        for i in 0..conc {
                            let path =
                                CriticalPath::new(format!("concurrent_{}", i), config.clone());
                            let packet = vec![0u8; 4096];

                            let task = async move { path.process_packet(&packet).await };
                            tasks.push(task);
                        }
    ...
    c.bench_function("memory_allocation_patterns", |b| {
        ...
        b.iter(|| {
            rt.block_on(async {
                ...
                let path = manager
                    .create_critical_path("mem_bench".to_string())
                    .await
                    .unwrap();
...
fn bench_optimization_effectiveness(c: &mut Criterion) {
    ...
    for zero_copy_enabled in [false, true].iter() {
        let label = if *zero_copy_enabled {
            "enabled"
        } else {
            "disabled"
        };
