...
fn multipath_failover_and_rejoin() {
    ...
    for _ in 0..200 {
        let p = mgr.send_data(vec![0]).unwrap();
        seen_initial.insert(p.path_id);
    }
    assert_eq!(
        seen_initial.len(),
        3,
        "all paths should participate initially"
    );
    ...
    for _ in 0..400 {
        let p = mgr.send_data(vec![1]).unwrap();
        seen_after_degrade.insert(p.path_id);
    }
    ...
    let mut total = 0u32;
    for _ in 0..400 {
        let p = mgr.send_data(vec![9]).unwrap();
        if p.path_id == 2 {
            count2 += 1;
        }
        total += 1;
    ...
    for _ in 0..400 {
        let p = mgr.send_data(vec![2]).unwrap();
        seen_recovered.insert(p.path_id);
    }
    assert!(
        seen_recovered.contains(&2),
        "recovered path did not rejoin scheduling set"
    );
}
