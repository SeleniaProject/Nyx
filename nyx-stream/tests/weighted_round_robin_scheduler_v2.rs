...
use nyx_stream::scheduler_v2::{WeightedRoundRobinScheduler, SchedulerStats, PathInfo};
...
fn test_path_management() {
    ...
    assert!(scheduler.update_path(250, Duration::from_millis(50)).is_err()); // System path
...
fn test_weight_calculation_from_rtt() {
    ...
    scheduler.update_path(3, Duration::from_millis(100)).unwrap(); // Weight: 10
...
fn test_smooth_wrr_distribution() {
    ...
    assert!(path1_ratio > 0.60 && path1_ratio < 0.65, "Path 1 ratio: {}", path1_ratio);
    assert!(path2_ratio > 0.29 && path2_ratio < 0.34, "Path 2 ratio: {}", path2_ratio);
    assert!(path3_ratio > 0.04 && path3_ratio < 0.08, "Path 3 ratio: {}", path3_ratio);
...
fn test_path_activation_deactivation() {
    ...
    assert!(path1_selected && path2_selected, "Both paths should be selected initially");
    ...
    assert!(path1_selected && path2_selected, "Both paths should be selected after reactivation");
}
...
fn test_rtt_updates() {
    ...
    // Initial RTT
    scheduler.update_path(1, Duration::from_millis(100)).unwrap();
    ...
    scheduler.update_path(1, Duration::from_millis(200)).unwrap();
...
fn test_extreme_rtt_values() {
    ...
    // Very high RTT
    scheduler.update_path(2, Duration::from_millis(5000)).unwrap();
    let low_weight = scheduler.path_info().iter().find(|p| p.path_id == 2).unwrap().weight;
    assert!(low_weight >= 1); // Should be clamped to minimum
    
    // Zero RTT (edge case)
    scheduler.update_path(3, Duration::from_millis(0)).unwrap();
    let zero_rtt_weight = scheduler.path_info().iter().find(|p| p.path_id == 3).unwrap().weight;
...
fn test_weight_reset() {
    ...
    for path in info_after {
        assert_eq!(path.current_weight, 0, "Current weight should be 0 after reset");
    }
}
...
fn test_concurrent_path_operations() {
    ...
    for i in 1..=10 {
        scheduler.update_path(i, Duration::from_millis(i as u64 * 10)).unwrap();
    }
    ...
    let expected_paths: std::collections::HashSet<PathId> = [1,3,5,7,9].iter().copied().collect();
...
fn test_performance_characteristics() {
    ...
    for i in 1..=NUM_PATHS {
        scheduler.update_path(i as PathId, Duration::from_millis(i as u64)).unwrap();
    }
    ...
    assert!(duration.as_millis() < 200, "Selection took too long: {:?}", duration);
...
fn test_fairness_with_equal_weights() {
    ...
    for i in 1..=NUM_PATHS {
        scheduler.update_path(i, Duration::from_millis(RTT_MS)).unwrap();
    }
    ...
    for i in 1..=NUM_PATHS {
        ...
        assert!(diff <= tolerance as i32, 
               "Path {} got {} selections, expected ~{} (tolerance: {})", 
               i, count, expected_per_path, tolerance);
    }
}
...
fn test_scheduler_lifecycle() {
    ...
    scheduler.update_path(2, Duration::from_millis(200)).unwrap(); // Worsen path 2
    ...
    assert!(path1_count > 60, "Path 1 should be selected more often with better RTT");
...
mod integration_tests {
    ...
    fn test_thread_safety_simulation() {
        ...
        // Setup phase: Add paths
        {
            ...
            for i in 1..=5 {
                sched.update_path(i, Duration::from_millis(i as u64 * 20)).unwrap();
            }
        }
