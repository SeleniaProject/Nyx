...
use nyx_stream::capability::{Capability, CAP_PLUGIN_FRAMEWORK, negotiate, get_local_capabilitie_s};
...
fn test_plugin_header_cbor_encoding() {
	let __hdr = PluginHeader { id: PluginId(42), __flag_s: 0b1010_0001, _data: vec![1, 2, 3, 4, 5] };
...
fn test_plugin_frame_building_and_parsing() {
	let __hdr = PluginHeader { id: PluginId(7), __flag_s: 0, _data: b"hello".to_vec() };
	let __payload = b"payload-byte_s-for-plugin";
	...
	for ft in [
		...
		let __f = PluginFrame::new(ft, hdr.clone(), payload);
		let __serialized = f.to_cbor()?;
		let __parsed = PluginFrame::from_cbor(&serialized)?;
...
//    NOTE: Library doe_s not enforce a hard limit; thi_s test assert_s we can
//    encode/decode moderately large frame_s used by implementation_s.
#[test]
fn test_plugin_frame_size_limit_s() {
	let __hdr = PluginHeader { id: PluginId(9), __flag_s: 0, _data: vec![0u8; 256] };
	let __payload = vec![0xABu8; 64 * 1024]; // 64 KiB payload typical upper-bound for control/_data
...
fn test_plugin_framework_capabilitynegotiation() {
	let __local_cap_s = get_local_capabilitie_s();
	
	// Should contain plugin framework capability
	let __plugin_cap = local_cap_s.iter()
...
fn test_plugin_frameworknegotiation_succes_s() {
	let __local_supported = &[nyx_stream::capability::CAP_CORE, CAP_PLUGIN_FRAMEWORK];
...
fn test_plugin_frameworknegotiation_without_plugin_s() {
	let __local_supported = &[nyx_stream::capability::CAP_CORE]; // No plugin framework
	let __peer_cap_s = vec![
		Capability::required(nyx_stream::capability::CAP_CORE, vec![]),
