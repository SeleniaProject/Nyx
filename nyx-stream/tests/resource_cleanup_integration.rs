...
use nyx_stream::{CleanupConfig, NyxAsyncStream};
...
async fn test_force_cleanup_with_timeout() {
    ...
    for i in 0..5 {
        let _op_id = stream
            .register_operation(&format!("test_op_{}", i))
            .await
            .unwrap();
    }
...
async fn test_memory_leak_prevention() {
    ...
    let stats_after = stream.get_stream_stats().await;
    println!(
        "Buffer size before: {}, after: {}",
        initial_write_buffer, stats_after.write_buffer_size
    );
...
async fn test_drop_cleanup() {
...

    {
        ...
        for i in 0..3 {
            let _op_id = stream
                .register_operation(&format!("drop_test_op_{}", i))
                .await
                .unwrap();
        }
...
async fn test_sequential_operations() {
    ...
    for i in 0..5 {
        let op_id = stream
            .register_operation(&format!("sequential_op_{}", i))
            .await
            .unwrap();
        op_ids.push(op_id);
    }
