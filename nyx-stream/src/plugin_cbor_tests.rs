...
impl PluginCborTestFixture {
    ...
    async fn register_test_plugin(&self, plugin_id: PluginId) -> PluginInfo {
        ...
        registry.register(plugin_info.clone()).await.expect("Plugin registration");
        plugin_info
    }
}
...
async fn test_cbor_header_creation_and_validation() {
    ...
    let header = PluginHeader::new(12345, 0x42, plugin_data.clone()).expect("Valid header creation");
...
async fn test_cbor_reserved_plugin_id_rejection() {
    ...
    assert!(matches!(result, Err(PluginCborError::ReservedPluginId(0xFFFF0000))));
    
    // Test upper bound of reserved range
    let result = PluginHeader::new(0xFFFFFFFF, 0x00, data.clone());
    assert!(matches!(result, Err(PluginCborError::ReservedPluginId(0xFFFFFFFF))));
...
async fn test_cbor_serialization_roundtrip() {
    ...
    for (id, flags, data) in test_cases {
        ...
        assert!(cbor_bytes.len() <= MAX_CBOR_HEADER_SIZE, 
                "Serialized size {} exceeds limit {}", cbor_bytes.len(), MAX_CBOR_HEADER_SIZE);
...
async fn test_cbor_malformed_data_rejection() {
    ...
    for malformed_cbor in malformed_test_cases {
        ...
        assert!(matches!(result, Err(PluginCborError::InvalidFormat(_))), 
                "Expected InvalidFormat error for data: {:?}", malformed_cbor);
    }
}
...
async fn test_plugin_dispatcher_cbor_integration() {
    ...
    fixture.dispatcher.load_plugin(plugin_info).await.expect("Plugin loading");
    ...
    for frame_type in frame_types {
        let result = fixture.dispatcher.dispatch_plugin_frame(frame_type, cbor_frame_data.clone()).await;
        assert!(result.is_ok(), "Frame type 0x{:02X} dispatch should succeed", frame_type);
    }
...
async fn test_plugin_dispatcher_invalid_frame_type() {
    ...
    for frame_type in invalid_frame_types {
        let result = fixture.dispatcher.dispatch_plugin_frame(frame_type, cbor_frame_data.clone()).await;
        assert!(matches!(result, Err(DispatchError::InvalidFrameType(_))), 
                "Frame type 0x{:02X} should be rejected", frame_type);
    }
}
...
async fn test_plugin_dispatcher_unregistered_plugin() {
    ...
    let plugin_data = b"test data".to_vec();
    let plugin_header = PluginHeader::new(unregistered_plugin_id, 0x00, plugin_data).expect("Valid header");
    let cbor_frame_data = serialize_plugin_header(&plugin_header).expect("CBOR serialization");
    
    // Attempt to dispatch to unregistered plugin
    let result = fixture.dispatcher.dispatch_plugin_frame(FRAME_TYPE_PLUGIN_DATA, cbor_frame_data).await;
...
async fn test_plugin_message_types() {
    ...
    let handshake_msg = PluginMessage::new(FRAME_TYPE_PLUGIN_HANDSHAKE, plugin_header.clone(), raw_frame.clone());
    ...
    // Test data message
    let data_msg = PluginMessage::new(FRAME_TYPE_PLUGIN_DATA, plugin_header.clone(), raw_frame.clone());
    ...
    let control_msg = PluginMessage::new(FRAME_TYPE_PLUGIN_CONTROL, plugin_header.clone(), raw_frame.clone());
...
async fn test_plugin_dispatcher_permission_validation() {
    ...
    let result = fixture.dispatcher.dispatch_plugin_frame(FRAME_TYPE_PLUGIN_CONTROL, cbor_frame_data.clone()).await;
    assert!(matches!(result, Err(DispatchError::InsufficientPermissions(_))));
...
async fn test_concurrent_plugin_operations() {
    ...
    join_set.spawn(async move {
        for i in 1..=10 {
            ...
            fixture_clone.dispatcher.load_plugin(plugin_info).await.expect("Plugin loading");
        }
    });
    ...
    join_set.spawn(async move {
        ...
        for i in 1..=5 {
            ...
            let _ = fixture_clone.dispatcher.dispatch_plugin_frame(FRAME_TYPE_PLUGIN_DATA, cbor_frame_data).await;
        }
    });
