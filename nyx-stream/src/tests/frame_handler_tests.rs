...
async fn test_frame_serialization_roundtrip() -> Result<()> {
    ...
    assert_eq!(
        original_frame.header.stream_id,
        decoded_frame.header.stream_id
    );
...
async fn test_frame_handler_performance() -> Result<()> {
    ...
    // Reuse a single buffer to avoid per-iteration allocation overhead
    ...
    // Performance requirement: account for Windows timer/granularity differences and CI variance
    // Non-Windows: <10ms, Windows: <150ms (allow headroom for slower CI runners)
