...
use crate::{HpkeRekeyManager, RekeyPolicy, TxQueue};
...
async fn hpke_rekey_async_flush_sends_frames() {
    ...
    assert_eq!(
        q_owned.drain_rekey_frames().await.len(),
        1,
        "expected queued frame"
    );
...
async fn hpke_rekey_telemetry_counters_increment() {
    ...
    let mut found_initiated = false;
    let mut found_applied = false;
    for f in families {
        let name = f.get_name();
        if name == "nyx_hpke_rekey_initiated_total" {
        ...
        if name == "nyx_hpke_rekey_applied_total" {
            if let Some(m) = f.get_metric().get(0) {
                if m.get_counter().get_value() >= 1.0 {
                    found_applied = true;
                }
...
async fn hpke_rekey_grace_usage_counter() {
    ...
    let policy = RekeyPolicy {
        time_interval: std::time::Duration::from_secs(3600),
        packet_interval: 1,
        grace_period: std::time::Duration::from_millis(30),
        min_cooldown: std::time::Duration::from_millis(0),
    };
...
async fn hpke_rekey_failure_counter() {
    ...
    let mut mgr = HpkeRekeyManager::new(
        RekeyPolicy {
            time_interval: std::time::Duration::from_secs(3600),
    ...
    for f in families {
        if f.get_name() == "nyx_hpke_rekey_fail_total" {
            if let Some(m) = f.get_metric().get(0) {
                if m.get_counter().get_value() >= 1.0 {
                    fail_ok = true;
                }
