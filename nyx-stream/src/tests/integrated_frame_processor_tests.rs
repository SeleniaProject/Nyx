use crate::integrated_frame_processor::{IntegratedFrameProcessor, ProcessorConfig, ProcessingMetrics};
use crate::{Frame, FrameHeader, FrameType, FrameCodec};
...
async fn test_frame_reordering_basic() -> Result<()> {
    ...
    assert!(metrics.frames_reordered > 0, "Should have detected reordering");
...
async fn test_complex_reordering_scenario() -> Result<()> {
    ...
    assert!(!all_processed.is_empty(), "Should have processed some frames");
...
async fn test_multiple_streams_reordering() -> Result<()> {
    ...
    assert_eq!(stream1_results.len(), 2, "Should release both stream 1 frames");
    
    // Process frame 1 for stream 2
    let stream2_results = processor.process_frame(stream2_frame1).await?;
    assert_eq!(stream2_results.len(), 2, "Should release both stream 2 frames");
...
async fn test_buffer_processing() -> Result<()> {
    ...
    for (i, frame) in processed_frames.iter().enumerate() {
        assert_eq!(frame.header.seq, (i + 1) as u64, "Frames should be in order");
    }
...
async fn test_processing_timeout() -> Result<()> {
    ...
    let result = processor.process_buffer(data).await;
    let elapsed = start.elapsed();
    
    // Either it completes very quickly or times out
    assert!(elapsed < Duration::from_millis(100), "Should either complete quickly or timeout");
...
async fn test_buffer_status_monitoring() -> Result<()> {
    ...
    if let Some(&(count, expected_seq, window)) = buffer_status.get(&1) {
    ...
    if let Some(&(count, expected_seq, window)) = buffer_status.get(&2) {
...
async fn test_buffer_flushing() -> Result<()> {
    ...
    assert_eq!(flushed1[0].header.seq, 3, "First flushed frame should be seq 3");
    assert_eq!(flushed1[1].header.seq, 5, "Second flushed frame should be seq 5");
    
    // Flush stream 2
    let flushed2 = processor.flush_stream_buffer(2).await?;
    assert_eq!(flushed2.len(), 2, "Should flush 2 frames from stream 2");
    assert_eq!(flushed2[0].header.seq, 2, "First flushed frame should be seq 2");
    assert_eq!(flushed2[1].header.seq, 4, "Second flushed frame should be seq 4");
    
    // Verify buffers are empty
    let buffer_status = processor.get_buffer_status().await;
    assert!(buffer_status.is_empty(), "All buffers should be empty after flushing");
...
async fn test_frame_encoding() -> Result<()> {
    ...
    for (i, frame) in decoded_frames.iter().enumerate() {
        assert_eq!(frame.header.seq, (i + 1) as u64, "Frame sequence should match");
    }
...
async fn test_reordering_window_limits() -> Result<()> {
    ...
    let buffer_status = processor.get_buffer_status().await;
    assert_eq!(buffer_status.get(&1).unwrap().0, 1, "Should have 1 buffered frame");
