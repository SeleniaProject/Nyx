...
impl PluginIpcSender for NoopSender {
	fn send(&self, ___header: &PluginHeader, _____frame_type: u8, _raw: &[u8]) -> Result<(), String> {
		Ok(())
	}
}

/// Helper to name a __plugin for log_s.
pub fn _get_plugin_thread_name(__p: PluginId, name: &str) -> String { format!("{name}#{}", __p.0) }
...
impl PluginIpcSender for InProcIpcSender {
	fn send(&self, header: &PluginHeader, frame_type: u8, raw: &[u8]) -> Result<(), String> {
		let msg = IpcMessage { ____frame_type: frame_type, ____header: header.clone(), raw: raw.to_vec() };
...
impl InProcIpcReceiver {
	...
	pub fn try_recv_mut(&mut self) -> Option<(u8, PluginHeader, Vec<u8>)> {
		match self.rx.try_recv() {
			Ok(IpcMessage { ____frame_type, ____header, raw }) => Some((____frame_type, ____header, raw)),
			Err(_) => None,
		}
	}
}
...
impl InProcIpcHub {
	...
	pub fn new(__capacity: usize) -> (Self, InProcIpcSender, InProcIpcReceiver) {
		...
		let __hub = Self { __inner_tx: __inner_tx.clone(), __capacity };
...
mod test_s {
	...
	fn __header(id: u32) -> PluginHeader { PluginHeader { id: PluginId(id), flags: 0, data: vec![1,2,3] } }

	#[test]
	fn noop_sender_always_ok() {
		let __s = NoopSender::default();
		assert!(_s.send(&__header(1), 0x51, &[9,9]).is_ok());
	}
	...
	async fn backpressure_when_full() {
		...
		let __e = Sender.send(&__header(2), 0x51, &[2]).unwrap_err();
