...
pub fn get_plugin_thread_name(p: PluginId, name: &str) -> String {
    format!("{name}#{}", p.0)
}
...
mod tests {
    ...
    #[tokio::test]
    async fn inproc_send_and_recv_nonblocking() -> Result<(), Box<dyn std::error::Error>> {
    ...
    async fn reconnect_works_after_receiver_drop() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let _send_result = sender.send(&header(1), 0x51, &[1]);
        // Reconnect
        ...
        assert!(got.is_some());
        let (_t, h, raw) = got.ok_or("Expected Some value")?;
        assert_eq!(h.id.0, 2);
        assert_eq!(raw, vec![2]);
        Ok(())
    }
