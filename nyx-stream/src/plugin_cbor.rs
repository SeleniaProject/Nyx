...
impl From<DeserializeError<std::io::Error>> for PluginCborError {
    fn from(e: DeserializeError<std::io::Error>) -> Self {
        PluginCborError::InvalidFormat(CborError::from(e))
    }
}

impl From<SerializeError<std::io::Error>> for PluginCborError {
    fn from(e: SerializeError<std::io::Error>) -> Self {
        PluginCborError::InvalidFormat(CborError::from(e))
    }
}
...
impl PluginHeader {
    ...
    pub fn new(id: PluginId, flags: u8, data: Vec<u8>) -> Result<Self, PluginCborError> {
        ...
        if data.len() > MAX_PLUGIN_DATA_SIZE {
            return Err(PluginCborError::DataSizeExceeded(
                data.len(),
...
pub fn parse_plugin_header(cbor_data: &[u8]) -> Result<PluginHeader, PluginCborError> {
    ...
    if cbor_data.len() > MAX_CBOR_HEADER_SIZE {
        return Err(PluginCborError::HeaderSizeExceeded(
            cbor_data.len(),
...
mod tests {
    ...
    fn test_reserved_plugin_id_rejection() {
        ...
        assert!(matches!(
            result,
            Err(PluginCborError::ReservedPluginId(0xFFFF0001))
        ));
    }
