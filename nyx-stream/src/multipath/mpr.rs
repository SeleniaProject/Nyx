...
use super::scheduler::{PathId, PathMetric, WeightedScheduler};
...
impl MprState {
    ...
    pub fn new(path_s: &[(PathId, PathMetric)]) -> Self {
        Self {
            sched: Some(WeightedScheduler::new(path_s)),
        }
    }
    pub fn pick_path(&mut self) -> PathId {
        self.sched
            .as_mut()
            .map(|s| s.next_path())
            .unwrap_or(PathId(0))
