...
impl StreamMetrics {
    ...
    pub fn record_send(&self, bytes: usize, duration: Duration) {
        ...
        self.send_duration_nanos.fetch_add(duration.as_nanos() as u64, Ordering::Relaxed);
    }
    ...
    pub fn record_recv(&self, bytes: usize, duration: Duration) {
        ...
        self.recv_duration_nanos.fetch_add(duration.as_nanos() as u64, Ordering::Relaxed);
    }
    ...
    fn update_memory_usage(&self, delta: isize) {
        let current = if delta >= 0 {
            self.current_memory_usage.fetch_add(delta as usize, Ordering::Relaxed) + delta as usize
        } else {
            self.current_memory_usage.fetch_sub((-delta) as usize, Ordering::Relaxed) - (-delta) as usize
        };
...
impl BufferPool {
    ...
    pub fn get_stats(&self) -> PoolStats {
        PoolStats {
            ...
            total_capacity: self.small_buffers.iter().map(|b| b.capacity()).sum::<usize>() +
                           self.medium_buffers.iter().map(|b| b.capacity()).sum::<usize>() +
                           self.large_buffers.iter().map(|b| b.capacity()).sum::<usize>(),
        }
    }
}
