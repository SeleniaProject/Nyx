...
use nyx_telemetry::{
    inc_hpke_rekey_cooldown_suppressed, inc_hpke_rekey_grace_used, observe_hpke_key_lifetime,
};
use std::sync::Arc;
...
impl HpkeRekeyManager {
    ...
    pub fn set_grace_expiry_notifier(&mut self, f: Arc<dyn Fn() + Send + Sync>) {
        self.grace_notifier = Some(f);
    }
    ...
    fn maintenance(&mut self) {
        if let Some((_, exp)) = &self.previous_key {
            if Instant::now() > *exp {
                ...
                if let Some(cb) = &self.grace_notifier {
                    cb();
                }
            }
        }
    ...
    pub fn evaluate(&self) -> RekeyDecision {
        ...
        if elapsed >= self.policy.time_interval
            || self.packets_since_rekey >= self.policy.packet_interval
        {
    ...
    where
        F: FnMut(&SessionKey) -> Option<T>,
    {
        self.maintenance();
        if let Some(res) = attempt(&self.current_key) {
            return Some(res);
        }
    ...
    pub fn previous_key_active(&self) -> bool {
        self.previous_key
            .as_ref()
            .map(|(_, exp)| Instant::now() <= *exp)
            .unwrap_or(false)
    }
    pub fn last_rekey_elapsed(&self) -> Duration {
...
mod tests {
    ...
    fn install_and_grace() {
        let policy = RekeyPolicy {
            time_interval: Duration::from_secs(999),
            packet_interval: 999_999,
    ...
    fn grace_notifier_fires() {
        ...
        let f2 = flag.clone();
        mgr.set_grace_expiry_notifier(Arc::new(move || {
        ...
        assert!(
            ...
            "grace expiry notifier did not fire"
        );
    }
    ...
    fn grace_usage_increments_counter() {
        ...
        let mut ok = false;
        for f in families {
            if f.get_name() == "nyx_hpke_rekey_grace_used_total" {
                if let Some(m) = f.get_metric().get(0) {
                    if m.get_counter().get_value() >= 1.0 {
                        ok = true;
                    }
                }
            }
