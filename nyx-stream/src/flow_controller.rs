...
impl RttEstimator {
    ...
    pub fn update(&mut self, rtt_sample: Duration) {
        match self.srtt {
            ...
            Some(srtt) => {
                ...
                self.srtt = Some(Duration::from_secs_f64(
                    (1.0 - self.alpha) * srtt.as_secs_f64() + self.alpha * rtt_sample.as_secs_f64(),
                ));
            }
        }
        ...
        trace!(
            "RTT updated: sample={:?}, srtt={:?}, rttvar={:?}, rto={:?}",
            rtt_sample,
            srtt,
            self.rttvar,
            self.rto
...
impl CongestionController {
    ...
    pub fn on_ack(&mut self, acked_bytes: u32, is_duplicate: bool) {
        ...
        trace!(
            "Congestion control update: state={:?}, cwnd={}, ssthresh={}, acked={}",
            self.state,
            self.cwnd,
            self.ssthresh,
            acked_bytes
    ...
    pub fn on_loss(&mut self) {
        match self.state {
            ...
            _ => {
                ...
                self.state = CongestionState::CongestionAvoidance;
                debug!(
                    "Packet loss detected, cwnd={}, ssthresh={}",
                    self.cwnd, self.ssthresh
                );
            }
        }
    ...
    fn enter_fast_recovery(&mut self) {
        ...
        debug!(
            "Entered fast recovery, cwnd={}, ssthresh={}",
            self.cwnd, self.ssthresh
        );
    }
...
impl FlowWindow {
    ...
    pub fn on_data_sent(&mut self, bytes: u32) -> Result<(), FlowControlError> {
        if self.bytes_in_flight + bytes > self.window_size {
            return Err(FlowControlError::FlowControlViolation {
                message: format!(
                    "Attempted to send {} bytes but only {} available",
                    bytes,
        ...
        trace!(
            "Data sent: {} bytes, in_flight: {}, window: {}",
            bytes,
    ...
    pub fn on_ack_received(&mut self, acked_bytes: u32) {
        ...
        trace!(
            "ACK received: {} bytes, in_flight: {}, window: {}",
            acked_bytes,
            self.bytes_in_flight,
            self.window_size
        );
...
impl FlowController {
    ...
    pub fn can_send(&self, bytes: u32) -> bool {
        ...
        let congestion_available = self
            .congestion_controller
            .can_send(self.flow_window.bytes_in_flight())
            >= bytes;
    ...
    pub fn on_ack_received(&mut self, acked_bytes: u32, rtt: Duration, is_duplicate: bool) {
        ...
        debug!(
            "ACK processed: {} bytes, duplicate: {}, RTT: {:?}",
    ...
    pub fn on_packet_lost(&mut self, lost_bytes: u32) {
        ...
        warn!(
            "Packet loss detected: {} bytes, total losses: {}",
    ...
    pub fn adaptive_flow_control_update(&mut self, network_delay: Duration, packet_loss_rate: f64) {
        ...
        if network_delay < Duration::from_millis(50) && packet_loss_rate < 0.001 {
            ...
            let _ = self
                .flow_window
                .update_window(new_window.min(self.flow_window.max_window_size()));
            debug!(
                "Increased flow window to {} due to good conditions",
                new_window
            );
        }
        // Decrease window if high delay or high loss
            ...
            debug!(
                "Decreased flow window to {} due to poor conditions",
                new_window
            );
        }
    }
    ...
    pub fn dynamic_window_adjustment(&mut self) -> u32 {
        ...
        if self.should_apply_backpressure() {
            let backpressure_factor =
                1.0 - (self.send_buffer.len() as f64 / self.max_send_buffer as f64);
            let adjusted_window = (effective_window as f64 * backpressure_factor) as u32;
            debug!(
                "Applied backpressure: window {} -> {}",
                effective_window, adjusted_window
            );
    ...
    pub fn should_send_window_update(&self) -> bool {
        ...
        let window_usage =
            self.flow_window.bytes_in_flight() as f64 / self.flow_window.window_size() as f64;
        window_usage > 0.75
    }
