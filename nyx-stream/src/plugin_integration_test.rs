...
fn empty_header_bytes(id: PluginId) -> Vec<u8> {
...
#[tokio::test]
async fn e2e_nowait_with_retry_backoff() {
	let registry = Arc::new(PluginRegistry::new());
	let dispatcher = PluginDispatcher::new(registry.clone());
	let pid = PluginId(55);
	...
	loop {
		match dispatcher.dispatch_plugin_frame_nowait(FRAME_TYPE_PLUGIN_DATA, bytes.clone()).await {
			Ok(()) => break,
			Err(crate::plugin_dispatch::DispatchError::IpcSendFailed(_, _)) => {
				attempt += 1;
				if attempt > 50 { panic!("exceeded retry attempts under backpressure"); }
				sleep(delay).await;
...
#[tokio::test]
async fn e2e_reconnect_after_unload_reload() {
	let registry = Arc::new(PluginRegistry::new());
	let dispatcher = PluginDispatcher::new(registry.clone());
	let pid = PluginId(66);
	...
	let hdr = build_handshake_header_bytes(pid, &hs).unwrap();
	dispatcher.dispatch_plugin_frame(HANDSHAKE_FRAME_TYPE, hdr.clone()).await.unwrap();
	...
	match err { crate::plugin_dispatch::DispatchError::PluginNotRegistered(x) => assert_eq!(x, pid), e => panic!("{e:?}") }

	// Reload and dispatch again
	dispatcher.load_plugin(info).await.unwrap();
	dispatcher.dispatch_plugin_frame(HANDSHAKE_FRAME_TYPE, hdr).await.unwrap();
}

#[tokio::test]
async fn e2e_all_frame_types_with_permissions() {
	let registry = Arc::new(PluginRegistry::new());
	let dispatcher = PluginDispatcher::new(registry.clone());
	let pid = PluginId(77);
	...
	let hdr_hs = build_handshake_header_bytes(pid, &hs).unwrap();
	dispatcher.dispatch_plugin_frame(HANDSHAKE_FRAME_TYPE, hdr_hs).await.unwrap();

	// Data/Control/Error frames use empty header payload for simplicity
	let empty = empty_header_bytes(pid);
	dispatcher.dispatch_plugin_frame(FRAME_TYPE_PLUGIN_DATA, empty.clone()).await.unwrap();
	dispatcher.dispatch_plugin_frame(FRAME_TYPE_PLUGIN_CONTROL, empty.clone()).await.unwrap();
	dispatcher.dispatch_plugin_frame(FRAME_TYPE_PLUGIN_ERROR, empty.clone()).await.unwrap();
}
