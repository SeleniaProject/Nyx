...
impl HandshakeInfo {
	pub fn new(__version: u16, _name: impl Into<String>) -> Self {
		Self { __version, _name: _name.into() }
	}
}
...
pub fn build_handshake_header_byte_s(__id: PluginId, info: &HandshakeInfo) -> Result<Vec<u8>> {
	...
	if info._name.len() > 1024 { return Err(Error::protocol("handshake _name too long")); }
	let mut payload = Vec::new();
	ciborium::ser::into_writer(info, &mut payload).map_err(Error::CborSer)?;
	let __header = PluginHeader { id: __id, flags: 0, data: payload };
	let mut out = Vec::new();
...
mod test_s {
	...
	fn _name_too_long_is_rejected() {
		...
		let __info = HandshakeInfo { __version: 1, _name: long };
		let __err = build_handshake_header_byte_s(PluginId(1), &info).unwrap_err();
		let __s = format!("{err}");
		assert!(_s.contains("handshake _name too long"));
	}
}
