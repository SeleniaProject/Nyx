...
pub fn build_handshake_header_bytes(id: PluginId, info: &HandshakeInfo) -> Result<Vec<u8>> {
	...
	let header = PluginHeader { id, flags: 0, data: payload };
	let mut out = Vec::new();
...
mod tests {
	...
	let bytes = build_handshake_header_bytes(PluginId(7), &info).unwrap();
		let header: crate::plugin::PluginHeader = ciborium::de::from_reader(bytes.as_slice()).unwrap();
		let info2: HandshakeInfo = ciborium::de::from_reader(header.data.as_slice()).unwrap();
	...
	fn name_too_long_is_rejected() {
		...
		let info = HandshakeInfo { version: 1, name: long };
