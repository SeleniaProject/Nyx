...
use crate::{
    ...
    plugin_cbor::{parse_plugin_header, serialize_plugin_header, PluginCborError, PluginHeader},
...
impl PluginDispatcher {
    ...
    pub async fn dispatch_plugin_frame(
        ...
        if !registry.has_permission(plugin_id, required_permission) {
            ...
            stats.total_errors += 1;
            warn!(
                "Plugin {} lacks permission {:?} for frame type 0x{:02X}",
        ...
        let runtime_handle = runtimes.get(&plugin_id).ok_or(DispatchError::RuntimeError(
            plugin_id,
            "Runtime not found".to_string(),
        ))?;
        ...
        debug!(
            "Successfully dispatched frame type 0x{:02X} to plugin {}",
    ...
    pub async fn dispatch_message(
        ...
        self.dispatch_plugin_frame(FRAME_TYPE_PLUGIN_DATA, message)
            .await
    }
    ...
    pub async fn load_plugin(&self, plugin_info: PluginInfo) -> Result<(), DispatchError> {
        ...
        let join_handle = tokio::spawn(async move {
            info!(
                "Starting plugin runtime for {} (ID: {})",
            ...
            while let Some(plugin_message) = rx.recv().await {
                ...
                match Self::process_plugin_message(plugin_id, &plugin_message).await {
                    Ok(()) => {
                        debug!(
                            "Successfully processed message {} for plugin {}",
                    ...
                    Err(e) => {
                        ...
                        if error_count > 100 {
                            error!(
                                "Plugin {} has too many errors ({}), terminating",
            ...
            info!(
                "Plugin runtime for {} (ID: {}) terminated. Processed {} messages, {} errors",
    ...
    async fn process_plugin_message(
        ...
        match message.frame_type {
            ...
            _ => {
                warn!(
                    "Unknown plugin frame type 0x{:02X} for plugin {}",
    ...
    async fn process_handshake_message(
        ...
        info!(
            "Plugin {} completed handshake with {} bytes of data",
            plugin_id,
            message.plugin_header.data.len()
        );
        Ok(())
    }
    ...
    async fn process_control_message(
        ...
        debug!(
            "Plugin {} sent control message with flags 0x{:02X}",
    ...
    async fn process_error_message(
        ...
        error!(
            ...
            plugin_id,
            String::from_utf8_lossy(&message.plugin_header.data)
        );
        Ok(())
    }
}
