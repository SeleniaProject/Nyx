...
impl AsyncStream {
	pub fn new(config: AsyncStreamConfig) -> Self {
        ...
        let (_wire_back_tx, wire_back_rx) = mpsc::channel::<LinkMsg>(1024);		tokio::spawn(endpoint_task(config, cmd_rx, wire_tx, wire_back_rx));
...
async fn endpoint_task(
	...
	let mut mpr = config.multipath.as_ref().and_then(|s| if s.enable_multipath && s.paths.len() > 1 { Some(MprState::new(&s.paths)) } else { None });
	let retransmit_alt = config.multipath.as_ref().map(|s| s.retransmit_on_new_path).unwrap_or(false);
	...
	loop {
		...
		for k in keys {
			...
			if let Some(entry) = inflight.get_mut(&k) {
				if now.duration_since(entry.last_sent) >= rtt.rto() && entry.retries < config.max_retries {
...
mod tests {
	...
	async fn multipath_preserves_ordering_at_receiver() -> Result<(), Box<dyn std::error::Error>> {
		...
		let (a, b) = pair(ca, cb);
		for i in 0..50u32 { a.send(Bytes::from(format!("m-{i}"))).await?; }
		let mut out = Vec::new();
		while out.len() < 50 {
	...
	async fn pending_reorder_cap_is_enforced() -> Result<(), Box<dyn std::error::Error>> {
		...
		for i in 0..10u32 { a.send(Bytes::from(format!("x-{i}"))).await?; }
