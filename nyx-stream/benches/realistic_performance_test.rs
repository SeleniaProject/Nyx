...
async fn realistic_frame_processing_benchmark(
    ...
    for i in 0..num_frames {
        ...
        match frame_handler
            .process_frame_async(i as u64, frame_data.clone())
            .await
        {
            Ok(Some(processed)) => {
                // Simulate additional processing (checksum, validation, etc.)
                let _checksum = processed
                    .iter()
...
fn bench_realistic_scenarios(c: &mut Criterion) {
    ...
    for (size, _delay, name, network_delay) in scenarios {
        ...
        group.bench_with_input(
            BenchmarkId::new(
                "realistic_processing",
                format!("{}_{}us", name, network_delay),
            ),
            &(size, network_delay),
            |b, &(data_size, delay)| {
...
async fn comprehensive_performance_analysis() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (size, count, rtt, loss, description) in test_scenarios {
        ...
        println!(
            "  Frame Processing: {:.2} Mbps, {:.2}ms latency, {:.1} ops/ms",
        ...
        println!(
            "  Flow Control: {:.2} Mbps, {:.2}ms latency, {:.1} ops/ms",
    ...
    println!(
        "Achieved: {:.2} Mbps throughput, {:.2}ms latency",
        target_test.throughput_mbps, target_test.latency_ms
    );
    println!(
        "Targets:  {:.0} Mbps throughput, {:.0}ms latency",
    ...
    if throughput_ok && latency_ok {
        ...
        if !throughput_ok {
            println!(
                "   - Throughput: {:.1}% of target ({:.2}/{:.0} Mbps)",
                target_test.throughput_mbps / target_throughput_mbps * 100.0,
                target_test.throughput_mbps,
        ...
        if !latency_ok {
            println!(
                "   - Latency: {:.1}x target ({:.2}/{:.0} ms)",
                target_test.latency_ms / target_latency_ms,
                target_test.latency_ms,
                target_latency_ms
            );
        } else {
...
mod tests {
    ...
    async fn test_realistic_frame_processing() {
        ...
        assert!(
            metrics.cpu_efficiency > 0.1,
            "CPU efficiency too low: {}",
            metrics.cpu_efficiency
        );

        println!(
            "Realistic frame processing: {:.2} Mbps, {:.2} ms, {:.2} ops/ms",
    ...
    async fn test_realistic_flow_control() {
        ...
        println!(
            "Realistic flow control: {:.2} Mbps, {:.2} ms, {:.2} ops/ms",
            metrics.throughput_mbps, metrics.latency_ms, metrics.cpu_efficiency
        );
    }
}
