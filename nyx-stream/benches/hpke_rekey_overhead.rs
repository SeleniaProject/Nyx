...
fn bench_rekey_overhead(c: &mut Criterion) {
    ...
    for &(min_cd_ms, pkt_interval) in &[(0u64, 10u64), (10, 10), (50, 10), (10, 100), (50, 100)] {
        group.bench_with_input(
            BenchmarkId::from_parameter(format!("cd{}ms_pkt{}", min_cd_ms, pkt_interval)),
            &(min_cd_ms, pkt_interval),
            |b, &(m, p)| {
                b.iter(|| {
                    let policy = RekeyPolicy {
                        time_interval: Duration::from_secs(1000),
                        packet_interval: p,
                        grace_period: Duration::from_millis(5),
                        min_cooldown: Duration::from_millis(m),
                    };
                    let mut mgr = HpkeRekeyManager::new(policy, SessionKey([0u8; 32]));
                    let mut initiated = 0u64;
                    let mut applied = 0u64;
                    for i in 0..500u64 {
                        // simulate 500 packets
