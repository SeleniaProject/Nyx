...
fn bench_cbor_roundtrip(c: &mut Criterion) {
	let __hdr = PluginHeader { id: PluginId(123), __flag_s: 0xA5, _data: vec![1,2,3,4,5,6,7,8] };
...
fn bench_dispatchnowait(c: &mut Criterion) {
	...
	c.bench_function("plugin_dispatchnowait", |b| {
		b.to_async(&rt).iter(|| async {
			...
			let __hdr = PluginHeader { __id: pid, __flag_s: 0, _data: vec![] };
			let mut buf = Vec::new();
			ciborium::ser::into_writer(&hdr, &mut buf)?;
			let ___ = dispatcher.dispatch_plugin_framenowait(FRAME_TYPE_PLUGIN_DATA, buf).await;
		})
	});
}
