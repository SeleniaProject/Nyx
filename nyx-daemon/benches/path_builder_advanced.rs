...
use nyx_daemon::path_builder::{DhtPeerDiscovery, CachedPeerInfo, DiscoveryCriteria};
use nyx_daemon::pure_rust_dht_tcp::{PureRustDht, DhtConfig};

/// Create a large set of diverse test peers for benchmarking
fn create_benchmark_peers(count: usize) -> Vec<CachedPeerInfo> {
    let mut peers = Vec::with_capacity(count);
    let regions = vec!["us-east-1", "us-west-1", "eu-west-1", "eu-central-1", "asia-east-1", "asia-south-1"];
    ...
    for i in 0..count {
        ...
        let (lat, lon) = match region {
            &"us-east-1" => (40.0 + (i as f64 * 0.1) % 5.0, -75.0 + (i as f64 * 0.1) % 5.0),
            &"us-west-1" => (37.0 + (i as f64 * 0.1) % 5.0, -122.0 + (i as f64 * 0.1) % 5.0),
            &"eu-west-1" => (51.0 + (i as f64 * 0.1) % 5.0, -1.0 + (i as f64 * 0.1) % 5.0),
            &"eu-central-1" => (52.0 + (i as f64 * 0.1) % 5.0, 13.0 + (i as f64 * 0.1) % 5.0),
            &"asia-east-1" => (35.0 + (i as f64 * 0.1) % 5.0, 139.0 + (i as f64 * 0.1) % 5.0),
            _ => (0.0, 0.0),
        };
        
        let multiaddr: Multiaddr = format!("/ip4/10.{}.{}.{}/tcp/{}", 
                                          (i / 256) % 256, 
                                          i % 256, 
                                          (i * 3) % 256, 
                                          8080 + (i % 1000))
            ...
            |b, &_path_length| {
                b.iter(|| {
                    rt.block_on(async {
                        ...
                        let peer_discovery = DhtPeerDiscovery::new(dht).await.expect("DHT creation");
                        
                        peer_discovery.calculate_path_diversity_score(&test_peers).await
                    })
                })
            },
        );
            ...
            |b, &_candidate_count| {
                b.iter(|| {
                    rt.block_on(async {
                        ...
                        peer_discovery.optimize_peer_selection(&mut peers).await.unwrap()
                    })
                })
            },
        );
...
fn bench_diverse_path_selection(c: &mut Criterion) {
    ...
    for (candidates, path_len) in [(100, 3), (500, 5), (1000, 7)].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("select_diverse_peers", format!("{}c_{}p", candidates, path_len)),
            &(*candidates, *path_len),
            |b, &(candidates, path_len)| {
                b.iter(|| {
                    rt.block_on(async {
                        ...
                        peer_discovery.select_diverse_path_peers(&test_peers, path_len).await.unwrap()
                    })
                })
            },
        );
...
fn bench_advanced_peer_scoring(c: &mut Criterion) {
    ...
    let dht_config = DhtConfig::default();
    let dht = Arc::new(RwLock::new(Some(PureRustDht::new(dht_config))));
    let peer_discovery = rt.block_on(async {
            ...
            |b, &peer_count| {
                b.iter(|| {
                    rt.block_on(async {
                        ...
                        let selected = peer_discovery.select_diverse_path_peers(&test_peers, 5).await.unwrap();
                        let diversity_score = peer_discovery.calculate_path_diversity_score(&selected).await;
