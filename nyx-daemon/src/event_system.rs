...
impl EventSystem {
    ...
    async fn process_event_batch(&self) -> Result<()> {
        ...
        for prioritized_event in events_to_process {
            if let Err(e) = self.distribute_event(&prioritized_event.event).await {
                ...
                if prioritized_event.priority == EventPriority::Critical
                    && prioritized_event.retry_count < 3
                {
    ...
    async fn distribute_event(&self, event: &Event) -> Result<()> {
        ...
        for (subscriber_id, subscriber) in subscribers.iter() {
            ...
            if subscriber.filter.matches(event) {
                ...
                let send_result = timeout(self.processing_timeout, async {
    ...
    async fn drop_low_priority_events(&self, queue: &mut VecDeque<PrioritizedEvent>) {
        ...
        if dropped_count > 0 {
            warn!(
                "Dropped {} low-priority events due to queue overflow",
                dropped_count
            );
        }
    ...
    async fn record_event(&self, event: &Event) {
        ...
        *stats.events_by_priority.entry(priority_str).or_insert(0) += 1;
        ...
        debug!(
            "Recorded event: type={}, severity={}, priority={:?}",
    ...
    pub(crate) async fn get_subscriber_info(&self) -> HashMap<String, (bool, u32, Option<String>)> {
        ...
        subscribers
            ...
            .map(|(id, sub)| {
                (
                    id.clone(),
                    (sub.is_active, sub.error_count, sub.last_error.clone()),
                )
            })
            .collect()
    }
