...
impl HealthMonitor {
    ...
    pub async fn start(&self) -> anyhow::Result<()> {
        ...
        info!(
            "Health monitor started with {} second intervals",
        ...
        if let Ok(mut funcs) = self.check_functions.try_write() {
            ...
            let _ = tokio::task::spawn(async move {
                check_functions
                    .write()
                    .await
                    .insert(name_clone, check_fn_box);
            });
        }
    ...
    async fn update_overall_status(&self) {
        ...
        debug!(
            "health_counts healthy={} degraded={} unhealthy={}",
...
mod tests {
    ...
    async fn test_health_monitor_creation() {
        ...
        let checks = monitor.get_all_checks().await;
        panic!(
            "default checks not registered in time: keys={:?}",
    ...
    async fn test_health_status_aggregation() {
        ...
        for _ in 0..10u8 {
            ...
            let checks = monitor.get_all_checks().await;
            if checks.len() >= 4
                && checks.contains_key("system_memory")
                && checks.contains_key("system_cpu")
            {
                break;
            }
            tokio::time::sleep(std::time::Duration::from_millis(50)).await;
