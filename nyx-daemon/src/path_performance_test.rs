...
mod tests {
    use super::super::path_builder::{
        GlobalPathStats, PathBuilder, PathBuilderConfig, PathPerformanceMetrics,
        PathPerformanceMonitor, PerformanceTrend,
    };
    use std::sync::Arc;
    ...
    async fn test_performance_trend_analysis() {
        ...
        for i in 0..15 {
            ...
            monitor.record_latency(latency).await;
            monitor
                .record_transmission(1000, if success { 1000 } else { 0 }, success)
                .await;
        ...
        assert!(matches!(
            trend,
            PerformanceTrend::Descending | PerformanceTrend::Volatile | PerformanceTrend::Unknown
        ));
    ...
    async fn test_performance_alerts() {
        ...
        monitor
            .set_alert_callback(move |metrics| {
        ...
        assert!(
            current_metrics.reliability_score < 0.5,
            "Reliability score {} should be < 0.5",
    ...
    async fn test_path_builder_performance_integration() {
        ...
        let result: Result<PathBuilder, anyhow::Error> =
            Ok(PathBuilder::new(bootstrap_peers, config));
    ...
    async fn test_global_stats() {
        ...
        assert_eq!(
            ...
            Some("path_1".to_string())
        );
        assert_eq!(
            global_stats.worst_performing_path,
            Some("path_3".to_string())
        );
    }
