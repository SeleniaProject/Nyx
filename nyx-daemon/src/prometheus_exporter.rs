...
use crate::metrics::{LayerStatus, LayerType, MetricsCollector};
...
impl PrometheusExporter {
    ...
    pub async fn start_collection(&self) -> Result<(), PrometheusError> {
        ...
        *running = true;
        ...
        info!(
            "Started Prometheus metrics collection with {}s interval",
    ...
    async fn export_basic_metrics(
        ...
        metrics::counter!("nyx_requests_total").absolute(
            collector
                .total_requests
                .load(std::sync::atomic::Ordering::Relaxed),
        );
        metrics::counter!("nyx_successful_requests_total").absolute(
            collector
                .successful_requests
                .load(std::sync::atomic::Ordering::Relaxed),
        );
        metrics::counter!("nyx_failed_requests_total").absolute(
            collector
                .failed_requests
                .load(std::sync::atomic::Ordering::Relaxed),
        );
        ...
        metrics::counter!("nyx_bytes_sent_total").absolute(
            collector
                .bytes_sent
                .load(std::sync::atomic::Ordering::Relaxed),
        );
        metrics::counter!("nyx_bytes_received_total").absolute(
            collector
                .bytes_received
                .load(std::sync::atomic::Ordering::Relaxed),
        );
        metrics::counter!("nyx_packets_sent_total").absolute(
            collector
                .packets_sent
                .load(std::sync::atomic::Ordering::Relaxed),
        );
        metrics::counter!("nyx_packets_received_total").absolute(
            collector
                .packets_received
                .load(std::sync::atomic::Ordering::Relaxed),
        );
        metrics::counter!("nyx_retransmissions_total").absolute(
            collector
                .retransmissions
                .load(std::sync::atomic::Ordering::Relaxed),
        );
        ...
        metrics::gauge!("nyx_active_streams").set(
            collector
                .active_streams
                .load(std::sync::atomic::Ordering::Relaxed) as f64,
        );
        metrics::gauge!("nyx_connected_peers").set(
            collector
                .connected_peers
                .load(std::sync::atomic::Ordering::Relaxed) as f64,
        );
        metrics::counter!("nyx_total_connections").absolute(
            collector
                .total_connections
                .load(std::sync::atomic::Ordering::Relaxed),
        );
        metrics::counter!("nyx_failed_connections").absolute(
            collector
                .failed_connections
                .load(std::sync::atomic::Ordering::Relaxed),
        );
        ...
        // Export performance metrics
        {
            ...
            metrics::gauge!("nyx_cover_traffic_rate_pps").set(*cover_traffic_rate);
        }

        {
            let avg_latency = collector.avg_latency_ms.read().await;
            metrics::gauge!("nyx_average_latency_ms").set(*avg_latency);
        }

        {
            let packet_loss_rate = collector.packet_loss_rate.read().await;
            metrics::gauge!("nyx_packet_loss_rate").set(*packet_loss_rate);
        }

        {
            let bandwidth_utilization = collector.bandwidth_utilization.read().await;
            metrics::gauge!("nyx_bandwidth_utilization").set(*bandwidth_utilization);
        }

        // Export uptime
        let uptime = collector.get_uptime();
        metrics::gauge!("nyx_uptime_seconds").set(uptime.as_secs() as f64);

        // Export performance metrics from the collector
        let perf_metrics = collector.get_performance_metrics().await;
        metrics::gauge!("nyx_cpu_usage").set(perf_metrics.cpu_usage);
        metrics::gauge!("nyx_memory_usage_mb").set(perf_metrics.memory_usage_mb);
        metrics::gauge!("nyx_connection_success_rate").set(perf_metrics.connection_success_rate);

        // Export resource usage if available
        if let Some(resource_usage) = collector.get_resource_usage().await {
            metrics::gauge!("nyx_memory_rss_bytes").set(resource_usage.memory_rss_bytes as f64);
            metrics::gauge!("nyx_memory_vms_bytes").set(resource_usage.memory_vms_bytes as f64);
            metrics::gauge!("nyx_cpu_percent").set(resource_usage.cpu_percent);
            ...
            metrics::gauge!("nyx_memory_bytes").set(resource_usage.memory_bytes as f64);
            metrics::gauge!("nyx_open_fds").set(resource_usage.open_file_descriptors as f64);
            metrics::gauge!("nyx_threads").set(resource_usage.thread_count as f64);
        }
        ...
        metrics::gauge!("nyx_mix_routes_count").set(mix_routes.len() as f64);
        ...
        if !memory_samples.is_empty() {
            ...
            metrics::gauge!("nyx_memory_average_percent").set(avg_memory);
        }
    ...
    async fn export_dashboard_summary(&self) -> Result<(), PrometheusError> {
        ...
        metrics::gauge!("nyx_dashboard_health_score").set(health_score.max(0.0).min(1.0));

        // Export connection success rate
        let total_attempts = collector
            .connection_attempts
            .load(std::sync::atomic::Ordering::Relaxed);
        let successful = collector
        ...
        metrics::gauge!("nyx_dashboard_connection_success_rate").set(success_rate);
    ...
    async fn export_health_indicators(&self) -> Result<(), PrometheusError> {
        ...
        let high_cpu = if perf_metrics.cpu_usage > 0.8 {
            1.0
        } else {
            0.0
        };
        metrics::gauge!("nyx_health_high_cpu_usage").set(high_cpu);

        let high_latency = if perf_metrics.avg_latency_ms > 1000.0 {
            1.0
        } else {
            0.0
        };
        metrics::gauge!("nyx_health_high_latency").set(high_latency);

        let high_packet_loss = if perf_metrics.packet_loss_rate > 0.05 {
            1.0
        } else {
            0.0
        };
        metrics::gauge!("nyx_health_high_packet_loss").set(high_packet_loss);

        let low_connection_success = if perf_metrics.connection_success_rate < 0.9 {
            1.0
        } else {
            0.0
        };
        metrics::gauge!("nyx_health_low_connection_success").set(low_connection_success);
