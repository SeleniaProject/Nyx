...
pub enum PushResult { Accepted, Queued, Rejected(String) }
...
pub struct FcmProvider { pub project_id: String, pub accept: bool }
impl PushProvider for FcmProvider {
	fn send(&self, _topic: &str, _payload: &str, _timestamp_ms: u128) -> PushResult {
		if self.accept { PushResult::Accepted } else { PushResult::Rejected("FCM provider not accepting".into()) }
	}
	fn healthy(&self) -> bool { self.accept }
}

/// A no-op APNS provider implementation (pure Rust, no external deps).
pub struct ApnsProvider { pub team_id: String, pub accept: bool }
impl PushProvider for ApnsProvider {
	fn send(&self, _topic: &str, _payload: &str, _timestamp_ms: u128) -> PushResult {
		if self.accept { PushResult::Accepted } else { PushResult::Rejected("APNS provider not accepting".into()) }
	}
	fn healthy(&self) -> bool { self.accept }
}
...
impl PushManager {
    ...
    pub fn new(max_queue: usize) -> Self {
        Self { inner: Arc::new(Mutex::new(PushManagerInner {
            queue: VecDeque::new(),
            max_queue,
	...
	pub fn bind_topic(&self, topic: &str, provider_name: &str) {
		let mut g = self.inner.lock().unwrap();
		g.topic_routes.insert(topic.to_string(), provider_name.to_string());
	}
    ...
    pub fn publish(&self, topic: &str, payload: &str) {
        ...
        if !routed { self.enqueue(topic, payload, ts, 0); }
	}
    ...
    fn enqueue(&self, topic: &str, payload: &str, timestamp_ms: u128, attempts: u32) {
        ...
        g.queue.push_back(QueuedItem { msg: PushMessage { topic: topic.into(), payload: payload.into(), timestamp_ms }, attempts, next_attempt_at: next });
	}
            ...
            loop {
                ...
                let work = {
                    ...
                    if let Some(front) = g.queue.front() {
                        if front.next_attempt_at > Instant::now() { None } else { Some(g.queue.pop_front().unwrap()) }
                    } else { None }
                };
                if let Some(item) = work {
                    ...
                    let (provider_opt, retry_cfg) = {
                        ...
                        let provider = g.topic_routes.get(&item.msg.topic).and_then(|name| g.providers.get(name)).cloned();
                        (provider, g.retry_config)
                    };
                    if let Some(provider) = provider_opt {
                        match provider.send(&item.msg.topic, &item.msg.payload, item.msg.timestamp_ms) {
                            PushResult::Accepted => {
                            ...
                            PushResult::Rejected(err) => {
                                warn!("push rejected by provider: topic={} err={} attempts={}", item.msg.topic, err, item.attempts);
                                mgr.requeue_with_backoff(item, retry_cfg);
                            }
                        }
                    } else {
    ...
    fn requeue_with_backoff(&self, mut item: QueuedItem, cfg: RetryConfig) {
        ...
        let backoff_ms = ((cfg.base_delay_ms as u128) * (1u128 << item.attempts.min(cfg.max_exponent) as u128))
...
impl Default for RetryConfig {
    fn default() -> Self {
        Self { base_delay_ms: 250, max_delay_ms: 30_000, max_exponent: 8, jitter: 0.1 }
    }
}
