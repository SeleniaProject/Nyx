...
use crate::metrics::{
    AlertSeverity, AlertThreshold, ErrorMetrics, LayerMetrics, LayerType, MetricsSnapshot,
    NetworkMetrics, SuppressionRule, SystemMetrics, ThresholdComparison,
};

/// Test the enhanced alert system functionality
pub async fn test_alert_system() {
    tracing::info!(
        target = "nyx-daemon::alert_system_test",
...
async fn test_threshold_monitoring(alert_system: &EnhancedAlertSystem) {
    ...
    tracing::info!(
        target = "nyx-daemon::alert_system_test",
        "Threshold monitoring working correctly"
    );
}
...
async fn test_alert_routing(alert_system: &EnhancedAlertSystem) {
    tracing::info!(
        target = "nyx-daemon::alert_system_test",
        "Testing alert routing and priority..."
    );
    ...
    for alert in &warning_alerts {
        if alert.severity == AlertSeverity::Warning {
            ...
            assert!(
                ...
                "Warning alerts should have medium priority"
            );
        }
    }
    ...
    tracing::info!(
        target = "nyx-daemon::alert_system_test",
        "Alert routing and priority working correctly"
    );
}
...
async fn test_statistics_and_analysis(alert_system: &EnhancedAlertSystem) {
    ...
    assert!(
        !stats.active_by_severity.is_empty(),
    ...
    assert!(
        !report.metric_frequency.is_empty(),
        "Should have metric frequency data"
    );
    assert!(
    ...
    if let Some(alert) = alerts.first() {
        ...
        assert!(
            updated_stats.total_resolved > 0,
            "Should have resolved alerts count"
        );
    }

    tracing::info!(
        target = "nyx-daemon::alert_system_test",
        "Statistics and analysis working correctly"
    );
}
