...
use nyx_mobile_ffi::{
    nyx_mobile_init, nyx_mobile_set_log_level, nyx_mobile_shutdown, nyx_power_get_state, NyxPowerState, NyxStatus,
...
impl LowPowerBridge {
    ...
    pub fn start(events: EventSystem) -> anyhow::Result<Self> {
        ...
        if let Some(lv) = std::env::var("NYX_MOBILE_LOG_LEVEL").ok() {
            ...
            let _ = nyx_mobile_set_log_level(code);
        }
    ...
    let handle = tokio::spawn(async move {
            ...
            let start_ms = now_ms();
            let mut inactivity = InactivityTrigger::new(Duration::from_millis(inactivity_ms), rate_per_sec, TimestampMs(start_ms));
            ...
            loop {
                ...
                if prev_state != Some(cur) {
                    ...
                    // Emit rich event
                    let detail = serde_json::to_string(&PowerEvent::State { state: display_power(cur).to_string() }).unwrap_or_else(|_| "{\"type\":\"state\"}".into());
                    let _ = events.sender().send(Event { ty: "power".into(), detail });
                    // Metrics
                    metrics::counter!("nyx.power.state.change", "state" => cur.to_string()).increment(1);
                ...
                if inactivity.should_trigger(TimestampMs(now)) {
                    let detail = serde_json::to_string(&PowerEvent::Inactivity).unwrap_or_else(|_| "{\"type\":\"inactivity\"}".into());
                    let _ = events.sender().send(Event { ty: "power".into(), detail });
                ...
                if wk > prev_wake {
                    let detail = serde_json::to_string(&PowerEvent::Wake).unwrap_or_else(|_| "{\"type\":\"wake\"}".into());
                    let _ = events.sender().send(Event { ty: "power".into(), detail });
                    prev_wake = wk;
                }
                let rs = rust_get_resume_count();
                if rs > prev_resume {
                    let detail = serde_json::to_string(&PowerEvent::Resume).unwrap_or_else(|_| "{\"type\":\"resume\"}".into());
                    let _ = events.sender().send(Event { ty: "power".into(), detail });
                    prev_resume = rs;
                }
            }
...
mod tests {
    ...
    async fn emits_initial_state_event() {
        ...
        let _bridge = LowPowerBridge::start(events).expect("start bridge");
    ...
    async fn emits_on_state_change() {
        ...
        let events = EventSystem::new(16);
        let mut rx = events.subscribe();
        let _bridge = LowPowerBridge::start(events.clone()).expect("start bridge");
        ...
        let ev = timeout(Duration::from_millis(1500), rx.recv())
            .await
            .expect("event after state change")
            .expect("receive");
        assert_eq!(ev.ty, "power");
    assert!(ev.detail.contains("\"state\":\"background\""), "got {}", ev.detail);
    }
}
