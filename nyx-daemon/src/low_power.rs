...
use nyx_mobile_ffi::{
    nyx_mobile_get_app_state, nyx_mobile_get_battery_level, nyx_mobile_is_low_power_mode,
...
impl LowPowerManager {
    ...
    pub fn with_event_system(
        mut self: Arc<Self>,
        es: crate::event_system::EventSystem,
    ) -> Arc<Self> {
    ...
    pub fn set_manual_low_power(&self, enable: bool) {
        self.manual_override.store(enable, Ordering::Relaxed);
        let _ = self.evaluate();
    }
    ...
    fn read_mobile_power(&self) -> (Option<u8>, Option<bool>, Option<bool>, Option<AppLifecycle>) {
        ...
        let map_opt_u8 = |v: i32| {
            if v >= 0 {
                Some((v as u8).min(100))
            } else {
                None
            }
        };
    ...
    fn evaluate(&self) -> PowerState {
        ...
        if let Some(a) = app_state_opt {
            *self.app_state.lock() = a;
        }
        let app_state = *self.app_state.lock();
        let inactivity_threshold = match app_state {
            AppLifecycle::Active => self.config.inactivity_seconds,
            AppLifecycle::Background => self.config.inactivity_seconds / 2,
            AppLifecycle::Inactive => self.config.inactivity_seconds / 3,
        };
        let inactivity = self.last_activity.elapsed() > Duration::from_secs(inactivity_threshold);
        let env_battery_low = std::env::var("NYX_BATTERY_PERCENT")
            .ok()
            .and_then(|v| v.parse::<u8>().ok())
            .map(|b| b <= self.config.battery_threshold_percent)
            .unwrap_or(false);
        let battery_low = battery_level
            .map(|b| b <= self.config.battery_threshold_percent)
            .unwrap_or(env_battery_low);
        ...
        let trigger = manual
            || battery_low
            || os_low
            || (inactivity && (screen_off || app_state != AppLifecycle::Active));
        let new_state = if trigger {
            PowerState::Low
        } else {
            PowerState::Normal
        };
        self.dynamic_low_power
            .store(new_state == PowerState::Low, Ordering::Relaxed);
            ...
            *last = new_state;
            if let Some(es) = &self.event_system {
                if crate::event_system::EventSystem::is_enabled() {
                    let mut ev = crate::event_system::EventSystem::build_simple_event("power.state.changed", "info", format!("state={:?} battery_low={} inactivity={} screen_off={} os_low={} manual={} app_state={:?}", new_state, battery_low, inactivity, screen_off, os_low, manual, app_state));
                    es.publish_event(ev);
                }
            }
        }
    ...
    pub fn is_low_power(&self) -> bool {
        self.dynamic_low_power.load(Ordering::Relaxed)
    }
    pub fn recommended_cover_ratio(&self) -> f32 {
    ...
    pub async fn run(self: Arc<Self>) {
        ...
        loop {
            let app_state = *self.app_state.lock();
            let interval = match app_state {
                AppLifecycle::Active => 30,
                AppLifecycle::Background => 15,
                AppLifecycle::Inactive => 10,
            };
