...
async fn test_advanced_active_bandwidth_probe() {
    // Start mock server
    let mock_server = MockProbeServer::new()
        .await
        .expect("Failed to create mock server");
    let server_addr = mock_server
        .local_addr()
    ...
    assert!(
        probe_result.is_ok(),
        "Active bandwidth probe should succeed"
    );
    ...
    assert!(
        rtt > 0.0 && rtt < 5000.0,
        "RTT should be in reasonable range (0-5000ms)"
    );
    assert!(bandwidth >= 0.0, "Bandwidth should be non-negative");

    println!(
        "Active probe results: RTT={:.2}ms, Bandwidth={:.2}Mbps",
        rtt, bandwidth
    );
}
...
async fn test_advanced_diversity_optimization() {
    ...
    let selected_peers = peer_discovery
    ...
    assert!(
        regions.len() >= 2,
        "Selected peers should span multiple regions: {:?}",
        regions
    );
    ...
    let performance_tiers: HashSet<u8> = selected_peers
        .iter()
        .map(|peer| {
            get_performance_tier_for_test(&CachedPeerInfo {
                peer: peer.clone(),
                cached_at: Instant::now(),
                access_count: 0,
                last_accessed: Instant::now(),
...
async fn test_path_diversity_score_calculation() {
    ...
    // Verify score ranges
    assert!(
        diverse_score >= 0.0 && diverse_score <= 1.0,
        "Diversity score should be in [0,1] range"
    );
    assert!(
        similar_score >= 0.0 && similar_score <= 1.0,
        "Diversity score should be in [0,1] range"
    );
    ...
    assert!(
        diverse_score > 0.6,
        "Highly diverse path should score above 0.6"
    );

    println!(
        "Diversity scores - Diverse path: {:.3}, Similar path: {:.3}",
        diverse_score, similar_score
    );
}
...
async fn test_performance_optimization_integration() {
    ...
    let mut test_peers = vec![
        ...
        create_test_peer(
            ...
            Some(10.0),
            Some(500.0), // Poor static metrics
            vec!["onion-routing"],
        ),
    ...
    test_peers.sort_by(|a, b| {
        a.peer
            .bandwidth_mbps
            .partial_cmp(&b.peer.bandwidth_mbps)
            .unwrap_or(std::cmp::Ordering::Equal)
            .reverse()
    });

    // Find the responsive peer and verify it was probed
    let responsive_peer = test_peers
        .iter()
...
async fn test_advanced_path_builder_integration() {
    ...
    let test_peers = vec![
        ...
        create_test_peer(
            ...
            "us-west-1",
            Some(Point::new(37.7749, -122.4194)),
            Some(60.0),
            Some(300.0),
            vec!["onion-routing"],
        ),
        create_test_peer(
            ...
            "eu-central-1",
            Some(Point::new(52.5200, 13.4050)),
            Some(120.0),
            Some(250.0),
            vec!["exit-node"],
        ),
    ...
    assert!(
        min_distance > 500.0,
        "Minimum inter-peer distance should exceed 500km"
    );
    ...
    assert!(
        diversity_score > 0.5,
        "Path should achieve reasonable diversity score"
    );
    ...
    println!(
        "Performance diversity: {} tiers represented",
